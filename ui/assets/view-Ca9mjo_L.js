import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              *//* empty css              *//* empty css                 */import{o as lt,c as Ze,e as Ie}from"./utils-xPDMCTXC.js";import{s as qe}from"./notifications-CBkElf_0.js";import{i as gn,l as Si}from"./external-exe-DRsy67iY.js";import{c as wn,i as Ei,s as xi,t as Ci}from"./webcontent-client-DJU5VCvB.js";import{n as gt,R as Rn,i as Bt,c as Ii,I as At}from"./app-env-Dw5Rxq_p.js";import{S as et,P as ye}from"./app-runtime-CuOCpSBc.js";function ki({state:e,constants:n,escapeHtml:t,showToast:o,isPerfEnabled:r,formatDownloadBytes:u,formatDownloadSpeed:v,formatDownloadEta:x,formatHistoryTime:B,loadManagedDownloadsFromMain:L,refreshExtensionsManagerList:y,saveProfileHistoryStore:C,getActiveTab:k,updateTabTitle:$,createTab:R,switchTab:F,navigateTo:W}){const{PROFILES:Z,PARTITIONS:le,IS_DEV_APP_BUILD:q}=n;function se(f=""){const M=String(f||"").trim().toLowerCase();return["downloads","history","extensions","passwords"].includes(M)?M:"extensions"}function Se(f="extensions"){const M=se(f);return M==="downloads"?"Downloads":M==="history"?"History":M==="passwords"?"Passwords":"Extensions"}function ke(f="extensions"){const M=se(f);return M==="downloads"?"Ctrl+Shift+D":M==="history"?"Ctrl+H":M==="extensions"?"Ctrl+E":""}function nt(f){const M=f instanceof Element?f:null;return M?M.closest("input, textarea, select")?!0:M.isContentEditable===!0:!1}function Oe(f="settings",M="extensions"){return{type:String(f||"settings").trim().toLowerCase(),section:se(M)}}function ee(f="extensions"){const M=se(f);return e.tabs.find(E=>E?.nativePage?.type==="settings"&&se(E?.nativePage?.section)===M)||null}function ce(f){return f?.nativePage?.type==="settings"}async function Ke(){if(!window.api)return e.nativeSettingsGeneralInfo;try{if(!e.nativeSettingsGeneralInfo.version&&window.api.getAppVersion&&(e.nativeSettingsGeneralInfo.version=await window.api.getAppVersion()),!e.nativeSettingsGeneralInfo.defaultOpenStatus&&window.api.getDefaultOpenHandlingStatus){const f=await window.api.getDefaultOpenHandlingStatus();e.nativeSettingsGeneralInfo.defaultOpenStatus=f?.isDefault||f?.success?"Configured":"Needs setup"}}catch{}return e.nativeSettingsGeneralInfo}function Le(){return Object.entries(e.profileHistoryCache||{}).flatMap(([f,M])=>(Array.isArray(M)?M:[]).map(E=>({profileId:f,profileName:Z[f]?.name||f||"Unknown",url:String(E?.url||"").trim(),title:String(E?.title||"Untitled").trim()||"Untitled",visitedAt:E?.visitedAt?Number(E.visitedAt):0}))).filter(f=>f.url&&f.visitedAt).sort((f,M)=>Number(M.visitedAt||0)-Number(f.visitedAt||0))}function Ce(f){const M=new Date(Number(f||0));if(Number.isNaN(M.getTime()))return"Unknown Date";const E=new Date,A=new Date(E.getFullYear(),E.getMonth(),E.getDate()).getTime(),D=new Date(M.getFullYear(),M.getMonth(),M.getDate()).getTime(),X=A-1440*60*1e3;return D===A?"Today":D===X?"Yesterday":M.toLocaleDateString(void 0,{year:"numeric",month:"long",day:"numeric"})}function Pe(f=[]){const M=[],E=new Map;return f.forEach(A=>{const D=Ce(A.visitedAt);if(!E.has(D)){const X={key:`${D}-${A.visitedAt}`,label:D,entries:[]};E.set(D,X),M.push(X)}E.get(D).entries.push(A)}),M}function Ge(){return e.managedDownloadsCache.length?`
    ...
    `}async function xe(){const f=k();ce(f)&&he(f)}function Fe(f="extensions",M=e.activeTabId){const E=e.tabs.find(D=>D.id===M);if(!ce(E))return;const A=se(f);E.nativePage.section=A,$(E.id,Se(A)),E.id===e.activeTabId&&he(E)}function $e(f="extensions"){const M=ee(f);if(M){F(M.id);return}R(null,null,Se(f),null,{nativePage:Oe("settings",f)})}function Xe(){const f=document.getElementById("settingsBtn");f&&f.dataset.boundClick!=="1"&&(f.dataset.boundClick="1",f.addEventListener("click",()=>{$e("extensions")}))}function ot(){const f=document.getElementById("nativeTabContent");if(!f||f.dataset.boundNativeSettings==="1")return;f.dataset.boundNativeSettings="1",window.addEventListener("credentials-updated",()=>{e._credentialsLoaded=!1;const E=k();ce(E)&&E.nativePage?.section==="passwords"&&Je().then(()=>{he(E)})});const M=(E=null,A=null)=>{requestAnimationFrame(()=>{const D=document.getElementById("nativeHistorySearchInput");if(D&&(D.focus({preventScroll:!0}),Number.isInteger(E)&&Number.isInteger(A)&&typeof D.setSelectionRange=="function"))try{D.setSelectionRange(E,A)}catch{}})};f.addEventListener("click",async E=>{const A=E.target.closest(".password-visibility-toggle");if(A){E.preventDefault();const c=document.getElementById("nativePasswordSecretInput");if(c){const S=c.type==="password";c.type=S?"text":"password",A.innerHTML=S?`
      ...
      <div class="downloads-manager-list">
        ${e.managedDownloadsCache.length?e.managedDownloadsCache.map(p=>{const b=t(p.fileName||"Download"),O=String(p.state||"progressing"),Y=typeof p.progress=="number"?Math.max(0,Math.min(100,p.progress)):0,K=p.totalBytes>0?`${Re(p.receivedBytes)} / ${Re(p.totalBytes)}`:Re(p.receivedBytes),de=je(p.bytesPerSecond),ge=Je(p.etaSeconds),ue=O==="completed"?"Completed":O==="cancelled"?"Cancelled":O==="interrupted"?"Interrupted":p.paused?"Paused":`Downloading ${Y}%`,we=[de,ge].filter(Boolean).join(" • ");return`
    ...
    `}async function ot(a=null){const g=document.getElementById("downloadsManagerPanel"),m=document.getElementById("downloadsManagerBtn");if(!g||!m)return;if(!(a===null?g.classList.contains("hidden"):!!a)){$e();return}f(),Xe();try{await r(()=>g.classList.remove("hidden"),{captureSnapshots:!1}),R=!0}catch{g.classList.remove("hidden"),R=!1}m.classList.add("is-active")}function it(){let a=document.getElementById("downloadsShelf");a||(a=document.createElement("div"),a.id="downloadsShelf",a.className="downloads-shelf hidden",a.innerHTML=`
      ...
      `,document.body.appendChild(a),a.querySelector("#downloadsShelfClose")?.addEventListener("click",()=>{Z.visible=!1,it()}),a.querySelector("#downloadsShelfAction")?.addEventListener("click",async()=>{const b=he(Z.id);!b||!Z.action||!window.api?.runManagedDownloadAction||await window.api.runManagedDownloadAction({id:b.id,action:Z.action})}));const g=a.querySelector("#downloadsShelfTitle"),m=a.querySelector("#downloadsShelfMessage"),p=a.querySelector("#downloadsShelfAction");if(!Z.visible){a.classList.add("hidden");return}g&&(g.textContent="Downloads"),m&&(m.textContent=Z.message||""),p&&(p.textContent=Z.actionLabel||"Open",p.style.display=Z.action?"":"none"),a.classList.remove("hidden")}function ve(a={},g="updated"){const m=String(a.fileName||"Download").trim()||"Download";if(g==="created")Z={visible:!0,id:String(a.id||""),message:`${m} started downloading`,actionLabel:"Show",action:"show"};else if(g==="completed")Z={visible:!0,id:String(a.id||""),message:`${m} downloaded`,actionLabel:"Open",action:"open"};else if(g==="interrupted")Z={visible:!0,id:String(a.id||""),message:`${m} was interrupted`,actionLabel:"Retry",action:"retry"};else return;it(),W&&clearTimeout(W),W=setTimeout(()=>{Z.visible=!1,it()},5e3)}function f(){const a=document.getElementById("browserExtensionsMenu"),g=document.getElementById("browserExtensionsMenuBtn");a&&!a.classList.contains("hidden")&&(se?u(()=>a.classList.add("hidden")):a.classList.add("hidden")),se=!1,g&&g.classList.remove("is-active")}async function M(a=null){const g=document.getElementById("browserExtensionsMenu"),m=document.getElementById("browserExtensionsMenuBtn");if(!g||!m)return;if(!(a===null?g.classList.contains("hidden"):!!a)){f();return}A().catch(()=>{}),re();try{await r(()=>g.classList.remove("hidden"),{captureSnapshots:!1}),se=!0}catch(b){console.error("Failed to open extensions menu overlay",b),g.classList.remove("hidden"),se=!1}m.classList.add("is-active")}async function E(a,g="tab"){const m=e.browserExtensionsCache.find(Y=>Y.path===a);if(!m)return;const p=(m?.id||"guest").toLowerCase();let b="";if(g==="options"&&m.optionsUrl){const Y=m.manifest?.entrypoints?.options||"options.html";b=`${n.APP_PROTOCOL_SCHEME}://${p}/${Y}`}else if(g==="root"&&m.rootUrl){const Y=m.manifest?.entrypoints?.root||"result.html";b=`${n.APP_PROTOCOL_SCHEME}://${p}/${Y}`}else b=Ge(m);if(!b){o("This extension does not expose an openable page yet.","info");return}const O=`ext-${m.id||"guest"}`;B(b,O,`${m.name||"Extension"}${g==="options"?" Options":""}`,null,{extensionEntryPath:m.path,extensionActiveContext:ce()})}async function A(){if(window.api?.closeBrowserExtensionPopup)try{await window.api.closeBrowserExtensionPopup()}catch{}q={entryPath:"",pageType:"popup",host:null,overlayActive:!1},document.querySelectorAll(".browser-extension-action-btn").forEach(a=>a.classList.remove("is-active"))}function D(a={}){const g=document.getElementById("browserExtensionPopupTitle"),m=document.getElementById("browserExtensionPopupSubtitle"),p=document.getElementById("browserExtensionPopupIcon");g&&(g.textContent=Le(a)),m&&(m.textContent=a?.popupUrl?"Popup":a?.optionsUrl?"Extension page":a?.rootUrl?"Extension":""),p&&(p.innerHTML=Ve(a,"browser-extension-popup-fallback"))}function X(a){if(!a||typeof a.getBoundingClientRect!="function")return{left:0,top:0,bottom:0,width:0,height:0};const g=a.getBoundingClientRect();return{left:Number(g.left||0),top:Number(g.top||0),bottom:Number(g.bottom||0),width:Number(g.width||0),height:Number(g.height||0)}}async function te(a,g=null){const m=e.browserExtensionsCache.find(ge=>ge.path===a);if(!m)return;const p=String(m.popupUrl||"").trim()||String(m.optionsUrl||"").trim();if(!p){await E(a,"tab");return}if(q.entryPath===a){await A();return}await A(),f();const b=window.api?.openBrowserExtensionPopup;if(typeof b!="function")throw new Error("Extension popup API is unavailable");const O=ce(),Y=`ext-${m.id||"guest"}`,K=await b({url:p,entryPath:a,partition:Y,anchor:X(g),activeUrl:O.url||"",activeTitle:O.title||""});if(!K?.success)throw new Error(K?.message||"Could not open extension popup");D(m),q={entryPath:a,pageType:"popup",host:null,overlayActive:!1};const de=typeof CSS<"u"&&typeof CSS.escape=="function"?CSS.escape(a):a.replace(/["\\]/g,"\\$&");document.querySelectorAll(`.browser-extension-action-btn[data-path="${de}"]`).forEach(ge=>ge.classList.add("is-active"))}function Q(){const a=document.getElementById("browserExtensionsPinned");if(!a)return;const g=window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode,m=e.browserExtensionsCache.filter(b=>{const O=b.id==="sitesnap-studio"||String(b.name||"").toLowerCase().includes("sitesnap")||String(b.id||"").toLowerCase().includes("sitesnap");return g?b.enabled!==!1&&O:b.enabled!==!1&&!O}),p=g?m:m.filter(b=>Pe(b));if(!p.length){a.innerHTML="",a.classList.add("hidden");return}a.classList.remove("hidden"),a.innerHTML=p.map(b=>`
        ...
        `).join("")}function re(){const a=document.getElementById("browserExtensionsMenu");if(!a)return;const g=window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode,m=e.browserExtensionsCache.filter(p=>{const b=p.id==="sitesnap-studio"||String(p.name||"").toLowerCase().includes("sitesnap")||String(p.id||"").toLowerCase().includes("sitesnap");return g?p.enabled!==!1&&b:p.enabled!==!1&&!b});a.innerHTML=`
