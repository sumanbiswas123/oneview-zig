import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              *//* empty css              *//* empty css                 */import{c as gn,d as yi,s as Si,t as Ei,o as lt,b as Ze,e as Ce}from"./utils-BZXZwSp-.js";import{s as Ke}from"./notifications-CBkElf_0.js";import{i as wn,l as xi,q as Ci}from"./api-query-CnFqXUJs.js";import{n as gt,R as Ii,i as Bt,b as Li,I as At}from"./app-env-Cwx6WtbG.js";import{S as et,P as be}from"./app-runtime-CI2L9vDX.js";function ki({state:e,constants:n,escapeHtml:t,showToast:o,isPerfEnabled:r,formatDownloadBytes:u,formatDownloadSpeed:v,formatDownloadEta:x,formatHistoryTime:B,loadManagedDownloadsFromMain:P,refreshExtensionsManagerList:S,saveProfileHistoryStore:C,getActiveTab:L,updateTabTitle:D,createTab:R,switchTab:F,navigateTo:H}){const{PROFILES:ee,PARTITIONS:le}=n;function V(f=""){const T=String(f||"").trim().toLowerCase();return["downloads","history","extensions","passwords","about"].includes(T)?T:"extensions"}function ae(f="extensions"){const T=V(f);return T==="downloads"?"Downloads":T==="history"?"History":T==="passwords"?"Passwords":T==="extensions"?"Extensions":T==="about"?"About & Updates":"Extensions"}function Ie(f="extensions"){const T=V(f);return T==="downloads"?"Ctrl+Shift+D":T==="history"?"Ctrl+H":T==="extensions"?"Ctrl+E":""}function Le(f){const T=f instanceof Element?f:null;return T?T.closest("input, textarea, select")?!0:T.isContentEditable===!0:!1}function nt(f="settings",T="extensions"){return{type:String(f||"settings").trim().toLowerCase(),section:V(T)}}function je(f="extensions"){const T=V(f);return e.tabs.find(E=>E?.nativePage?.type==="settings"&&V(E?.nativePage?.section)===T)||null}function K(f){return f?.nativePage?.type==="settings"}async function me(){if(!window.api)return e.nativeSettingsGeneralInfo;try{if(!e.nativeSettingsGeneralInfo.version&&window.api.getAppVersion&&(e.nativeSettingsGeneralInfo.version=await window.api.getAppVersion()),!e.nativeSettingsGeneralInfo.defaultOpenStatus&&window.api.getDefaultOpenHandlingStatus){const f=await window.api.getDefaultOpenHandlingStatus();e.nativeSettingsGeneralInfo.defaultOpenStatus=f?.isDefault||f?.success?"Configured":"Needs setup"}}catch{}return e.nativeSettingsGeneralInfo}function ze(){return Object.entries(e.profileHistoryCache||{}).flatMap(([f,T])=>(Array.isArray(T)?T:[]).map(E=>({profileId:f,profileName:ee[f]?.name||f||"Unknown",url:String(E?.url||"").trim(),title:String(E?.title||"Untitled").trim()||"Untitled",visitedAt:E?.visitedAt?Number(E.visitedAt):0}))).filter(f=>f.url&&f.visitedAt).sort((f,T)=>Number(T.visitedAt||0)-Number(f.visitedAt||0))}function ke(f){const T=new Date(Number(f||0));if(Number.isNaN(T.getTime()))return"Unknown Date";const E=new Date,A=new Date(E.getFullYear(),E.getMonth(),E.getDate()).getTime(),M=new Date(T.getFullYear(),T.getMonth(),T.getDate()).getTime(),Q=A-1440*60*1e3;return M===A?"Today":M===Q?"Yesterday":T.toLocaleDateString(void 0,{year:"numeric",month:"long",day:"numeric"})}function Ee(f=[]){const T=[],E=new Map;return f.forEach(A=>{const M=ke(A.visitedAt);if(!E.has(M)){const Q={key:`${M}-${A.visitedAt}`,label:M,entries:[]};E.set(M,Q),T.push(Q)}E.get(M).entries.push(A)}),T}function Pe(){return e.managedDownloadsCache.length?`
    ...
    `:'<div class="native-settings-empty">No downloads yet.</div>'}function Je(){const f=e.historySearchQuery.trim().toLowerCase(),T=ze().filter(A=>f?`${A.title||""} ${A.url||""} ${A.profileName||""}`.toLowerCase().includes(f):!0);return T.length?`
    ...
    `}async function $e(){if(window.api?.listProfileCredentials)try{const f=await window.api.listProfileCredentials();if(f&&Array.isArray(f.data)){const T={wppproduction:[],vml:[],gsk:[],guest:[],synapse:[],contentgen:[]};f.data.forEach(E=>{const A=String(E.profileId||"").toLowerCase();T[A]||(T[A]=[]),T[A].push(E)}),e.credentialCache=T}}catch(f){console.error("loadCredentialsIntoCache error:",f)}}function xe(f){const T=document.getElementById("nativeTabContent");if(!T||!K(f))return;const E=V(f.nativePage?.section);let A="",M="";const Q=ae(E);let ie="Manage app behavior without leaving the browser shell.";if(E==="downloads"){const Z=e.managedDownloadsCache.length,oe=e.managedDownloadsCache.filter(d=>d.state==="progressing").length;A=Pe(),ie=`${oe} active, ${Z} total downloads.`}else if(E==="history"){const Z=ze(),oe=e.historySearchQuery.trim()?Z.filter(d=>`${d.title||""} ${d.url||""} ${d.profileName||""}`.toLowerCase().includes(e.historySearchQuery.trim().toLowerCase())).length:Z.length;M=`
      ...
      `,A=Je(),ie=`${oe} total history entries across all profiles.`}else if(E==="extensions"){const Z=e.browserExtensionsCache.filter(oe=>oe.enabled!==!1).length;M=`
        <div class="native-settings-toolbar">
          ${(function(){try{const oe=String(localStorage.getItem("userRole")||"production").toLowerCase(),d=String(localStorage.getItem("oneview_env_mode")||"prod").toLowerCase();return oe==="dev"&&d==="dev"?'<button class="native-settings-action" type="button" data-native-settings-action="load-unpacked-extension">Load unpacked extension</button>':""}catch{return""}})()}
        </div>
    ...
    `}function d(){Z(),oe()}function c(){const a=document.getElementById("extensionsManagerList");if(!a)return;const g=window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode,m=e.browserExtensionsCache.filter(p=>{const y=p.id==="sitesnap-studio"||String(p.name||"").toLowerCase().includes("sitesnap")||String(p.id||"").toLowerCase().includes("sitesnap");return g?y:!y});if(!m.length){a.innerHTML=`
      ...
      `;return}a.innerHTML=m.map(p=>{const y=t(p.path||"");return`
        ...
        `}).join("")}async function b(){try{await Ie()}catch(a){console.error("Failed to refresh browser extensions list",a)}c(),d()}async function I(a,g){const m=e.browserExtensionsCache.find(y=>y.path===a);if(!m)return;const p=g==="options"?m.optionsUrl:g==="root"?m.rootUrl:Je(m);if(!p){o(`This extension does not expose a ${g} page.`,"info");return}B(p,K(),`${m.name||"Extension"} ${g==="options"?"Options":"Popup"}`,null,{extensionEntryPath:m.path,extensionActiveContext:me()})}async function k(){const a=document.getElementById("extensionsManagerModal");if(a){f(),await A(),c(),d();try{await r(()=>a.classList.remove("hidden"))}catch(g){console.error("openOverlayModal failed for extensions manager",g),a.classList.remove("hidden")}b().catch(g=>{console.error("Failed to refresh extensions manager after open",g)})}}function O(){const a=document.getElementById("extensionsManagerModal");a&&u(()=>a.classList.add("hidden"))}function Y(){const a=document.getElementById("extensionsBtn"),g=document.getElementById("settingsBtn"),m=document.getElementById("extensionsModalCloseBtn"),p=document.getElementById("extensionsLoadBtn"),y=document.getElementById("extensionsManagerList"),U=document.getElementById("browserExtensionsPinned"),q=document.getElementById("browserExtensionsMenuBtn"),G=document.getElementById("browserExtensionsMenu"),ce=document.getElementById("downloadsManagerBtn"),ge=document.getElementById("downloadsManagerPanel"),de=document.getElementById("browserExtensionPopupClose"),we=document.getElementById("browserExtensionPopupOpenTab");if(nt(),c(),d(),b().catch(te=>{console.error("Failed to refresh extensions manager after open",te)}),Ie().then(()=>{if(Le(),window.api?.prewarmBrowserExtensionPopup){const ne=e.browserExtensionsCache.filter(X=>X.enabled!==!1).filter(X=>Pe(X));let ue={partition:K()};if(ne.length>0){const X=ne[0],pe=String(X.popupUrl||"").trim()||String(X.optionsUrl||"").trim();pe&&(ue={...ue,url:pe,entryPath:X.path})}window.api.prewarmBrowserExtensionPopup(ue).catch(()=>{})}}).catch(()=>{}),window.api?.onBrowserExtensionPopupState&&window.api.onBrowserExtensionPopupState(te=>{const{entryPath:ne,open:ue}=te||{};if(ue){V={entryPath:ne,pageType:"popup",host:null,overlayActive:!1};const X=typeof CSS<"u"&&typeof CSS.escape=="function"?CSS.escape(ne):ne.replace(/["\\]/g,"\\$&");document.querySelectorAll(`.browser-extension-action-btn[data-path="${X}"]`).forEach(pe=>pe.classList.add("is-active"))}else V.entryPath===ne&&(V={entryPath:"",pageType:"popup",host:null,overlayActive:!1},document.querySelectorAll(".browser-extension-action-btn").forEach(X=>X.classList.remove("is-active")))}),a&&a.dataset.boundClick!=="1"&&(a.dataset.boundClick="1",a.addEventListener("click",()=>{k().catch(te=>{console.error("Failed to open extensions manager",te),o("Could not open extensions manager.","error")})})),m&&m.dataset.boundClick!=="1"&&(m.dataset.boundClick="1",m.addEventListener("click",O)),p)try{const te=String(localStorage.getItem("userRole")||"production").toLowerCase(),ne=String(localStorage.getItem("oneview_env_mode")||"prod").toLowerCase();te!=="dev"||ne!=="dev"?p.style.display="none":p.style.display=""}catch{}p&&p.dataset.boundClick!=="1"&&(p.dataset.boundClick="1",p.addEventListener("click",async()=>{try{if(!window.api?.addBrowserExtensionsUnpacked){o("Extension manager API is unavailable.","error");return}const te=await window.api.addBrowserExtensionsUnpacked();e.browserExtensionsCache=Array.isArray(te?.entries)?te.entries:[],c(),d(),o("Unpacked extensions loaded.","success")}catch(te){console.error("Failed to load unpacked extensions",te),o(te?.message||"Could not load unpacked extensions.","error")}})),y&&y.dataset.boundClick!=="1"&&(y.dataset.boundClick="1",y.addEventListener("click",async te=>{const ne=te.target.closest("[data-action]");if(!ne)return;const ue=String(ne.dataset.action||"").trim(),X=String(ne.dataset.path||"").trim();if(X)try{if(ue==="open-popup"){await I(X,"popup");return}if(ue==="open-tab"){await I(X,"tab");return}if(ue==="open-options"){await I(X,"options");return}if(ue==="reload"&&window.api?.reloadBrowserExtension){const pe=await window.api.reloadBrowserExtension({path:X});e.browserExtensionsCache=Array.isArray(pe?.entries)?pe.entries:e.browserExtensionsCache,c(),d(),o("Extension reloaded.","success");return}if(ue==="toggle"&&window.api?.toggleBrowserExtension){const pe=e.browserExtensionsCache.find(vt=>vt.path===X);pe?.enabled!==!1&&typeof window.api?.closeBrowserExtensionPopup=="function"&&await window.api.closeBrowserExtensionPopup();const ct=await window.api.toggleBrowserExtension({path:X,enabled:pe?.enabled===!1});e.browserExtensionsCache=Array.isArray(ct?.entries)?ct.entries:e.browserExtensionsCache,c(),d(),o("Extension state updated.","success");return}if(ue==="remove"&&window.api?.removeBrowserExtension){typeof window.api?.closeBrowserExtensionPopup=="function"&&await window.api.closeBrowserExtensionPopup();const pe=await window.api.removeBrowserExtension({path:X});e.browserExtensionsCache=Array.isArray(pe?.entries)?pe.entries:e.browserExtensionsCache,c(),d(),o("Extension removed.","success")}}catch(pe){console.error("Extension manager action failed",pe),o(pe?.message||"Could not complete extension action.","error")}})),U&&U.dataset.boundClick!=="1"&&(U.dataset.boundClick="1",U.addEventListener("click",async te=>{const ne=te.target.closest("[data-path]");if(!ne)return;const ue=String(ne.dataset.path||"").trim();if(ue)try{await ie(ue,ne)}catch(X){console.error("Failed to open extension popup",X),o(X?.message||"Could not open extension popup.","error")}})),q&&q.dataset.boundClick!=="1"&&(q.dataset.boundClick="1",q.addEventListener("click",()=>{T().catch(te=>{console.error("Failed to toggle extensions menu",te),o("Could not open extensions menu.","error")})})),G&&G.dataset.boundClick!=="1"&&(G.dataset.boundClick="1",G.addEventListener("click",async te=>{const ne=te.target.closest("[data-menu-action]");if(!ne)return;const ue=String(ne.dataset.menuAction||"").trim(),X=String(ne.dataset.path||"").trim();try{if(ue==="manage"){f(),await k();return}if(!X)return;if(ue==="pin"){const pe=!le[X];le[X]=pe,je(),d();return}if(ue==="popup"){await ie(X,q||ne);return}if(ue==="options"){f(),await E(X,"options");return}ue==="tab"&&(f(),await E(X,"tab"))}catch(pe){console.error("Extension menu action failed",pe),o(pe?.message||"Could not complete extension action.","error")}})),de&&de.dataset.boundClick!=="1"&&(de.dataset.boundClick="1",de.addEventListener("click",()=>{A().catch(()=>{})})),we&&we.dataset.boundClick!=="1"&&(we.dataset.boundClick="1",we.addEventListener("click",async()=>{V.entryPath&&(await E(V.entryPath,"tab"),await A())})),document.body&&document.body.dataset.boundExtensionUiDismiss!=="1"&&(document.body.dataset.boundExtensionUiDismiss="1",document.addEventListener("click",te=>{const ne=te.target;g&&!g.contains(ne)&&S(),G&&!G.classList.contains("hidden")&&!G.contains(ne)&&!q?.contains(ne)&&!a?.contains(ne)&&f(),ge&&!ge.classList.contains("hidden")&&!ge.contains(ne)&&!ce?.contains(ne)&&_e(),!ne.closest(".browser-extension-action-btn")&&!G?.contains(ne)&&A().catch(()=>{})}),document.addEventListener("keydown",te=>{te.key==="Escape"&&(S(),f(),_e(),A().catch(()=>{}))}))}function j(){const a=document.getElementById("downloadsManagerBtn"),g=document.getElementById("downloadsManagerPanel");a&&a.dataset.boundClick!=="1"&&(a.dataset.boundClick="1",a.addEventListener("click",()=>{ot().catch(()=>{})})),g&&g.dataset.boundClick!=="1"&&(g.dataset.boundClick="1",g.addEventListener("click",async m=>{const p=m.target.closest("[data-download-action]");if(!p)return;const y=String(p.dataset.downloadAction||"").trim(),U=String(p.dataset.downloadId||"").trim();if(y)try{if(!window.api?.runManagedDownloadAction)return;const q=await window.api.runManagedDownloadAction({id:U,action:y});Array.isArray(q?.downloads)?e.managedDownloadsCache=q.downloads:(y==="remove"||y==="clear-completed")&&await We(),Xe()}catch(q){o(q?.message||"Could not complete download action.","error")}})),We().then(()=>{Xe()}).catch(()=>{}),window.api&&typeof window.api.onDownloadManagerUpdated=="function"&&document.body?.dataset.boundDownloadManagerEvents!=="1"&&(document.body.dataset.boundDownloadManagerEvents="1",window.api.onDownloadManagerUpdated(m=>{e.managedDownloadsCache=Array.isArray(m?.downloads)?m.downloads:[],Xe(),P().catch(()=>{}),(m?.reason==="created"||m?.reason==="completed"||m?.reason==="interrupted")&&Se();const p=Ue(String(m?.focusId||"").trim());p&&he(p,String(m?.reason||"updated"))}))}return{closeBrowserExtensionPopup:A,closeBrowserExtensionsMenu:f,closeDownloadsManagerPanel:_e,formatDownloadBytes:Fe,formatDownloadEta:xe,formatDownloadSpeed:$e,initDownloadsManager:j,initExtensionsManager:Y,loadManagedDownloadsFromMain:We,refreshBrowserExtensionsUi:d,refreshExtensionsManagerList:b}}function Bi({state:e,constants:n,showToast:t,openOverlayModal:o,closeOverlayModal:r,renderProfilePillSelect:u,resolveCredentialScopeIdForTab:v,applyProfileSelection:x,getCurrentProfileId:B,escapeHtml:P}){const{AUTH_GATEWAY_HOSTS:S,RESOURCE_SERVICE_ORIGIN:C}=n,L=()=>e.activeTabId,D=()=>e.credentialCache,R=d=>{e.credentialCache=d},F=()=>e.activeHttpAuthChallenge,H=d=>{e.activeHttpAuthChallenge=d},ee=()=>e.credentialCacheRefreshedAt,le=d=>{e.credentialCacheRefreshedAt=d},V=()=>e.credentialCacheRefreshInFlight,ae=d=>{e.credentialCacheRefreshInFlight=d},Ie=new Map;function Le(){return{wppproduction:[],vml:[],gsk:[],guest:[],synapse:[],contentgen:[]}}function nt(){return Object.keys(Le())}function je(){return!!(window.api&&typeof window.api.listProfileCredentials=="function"&&typeof window.api.saveProfileCredential=="function"&&typeof window.api.deleteProfileCredential=="function")}function K(d=""){const c=String(d).trim().toLowerCase();if(!c)return"";try{const O=new URL(c),Y=String(O.hostname||"").trim().toLowerCase().replace(/^www\./,""),j=String(O.port||"").trim();return Y?!j||j==="80"||j==="443"?Y:`${Y}:${j}`:""}catch{}const b=c.replace(/^https?:\/\//,"").replace(/^www\./,"").split("/")[0];if(!b)return"";const I=b.lastIndexOf(":");if(I<=0)return b;const k=b.slice(I+1);return/^\d+$/.test(k)&&k!=="80"&&k!=="443"?b:b.slice(0,I)}function me(d=""){const c=String(d||"").trim().toLowerCase();if(!c)return"";const b=c.lastIndexOf(":");if(b<=0)return c;const I=c.slice(b+1);return/^\d+$/.test(I)?c.slice(0,b):c}function ze(d=""){const c=[];try{const b=new URL(String(d||"")),I=(O="")=>{if(O)try{const Y=new URL(String(O)),j=K(Y.host||Y.hostname||"");j&&c.push(j)}catch{const j=K(String(O||""));j&&c.push(j)}};I(b.host||b.hostname||""),["retURL","retUrl","returnUrl","TargetResource","targetResource","PartnerSpId","partnerSpId"].forEach(O=>{I(b.searchParams.get(O)||"")})}catch{}return[...new Set(c.filter(Boolean))]}function ke(d=""){const c=ze(d);if(c.length===0)return K(d);const b=c[0]||"";if(S.has(b)){const I=c.find(k=>k&&!S.has(k));if(I)return I}return b}function Ee(d=""){return S.has(K(d))}function Pe(d=""){const c=me(K(d));return c.endsWith(".veevavault.com")||c==="veevavault.com"||c.endsWith(".gskinternet.com")||c==="gskinternet.com"||c.endsWith(".gskpro.com")||c==="gskpro.com"}function Je(d,c,b=""){const I=String(c||"").trim().toLowerCase(),k=K(b);if(!d||!I)return null;const O=(D()[d]||[]).filter(Y=>{const j=K(Y.domain);return j&&j!==k&&!Ee(j)&&Pe(j)&&String(Y.username||"").trim().toLowerCase()===I});return O.sort((Y,j)=>K(j.domain).length-K(Y.domain).length),O[0]||null}function qe(d=""){const c=String(d||"").trim();return/^(true|false|null|undefined|yes|no|on|off|0|1)$/i.test(c)?"":c}async function Fe(){if(!window.api?.deleteProfileCredential)return;const d=[];nt().forEach(c=>{(D()[c]||[]).forEach(b=>{const I=K(b.domain);!Ee(I)||!Je(c,b.username,I)||d.push({profileId:c,domain:I,username:String(b.username||"").trim()})})}),d.length!==0&&(await Promise.allSettled(d.map(c=>window.api.deleteProfileCredential(c))),d.forEach(c=>{const b=D()[c.profileId]||[];D()[c.profileId]=b.filter(I=>!(K(I.domain)===c.domain&&String(I.username||"").trim().toLowerCase()===c.username.toLowerCase()))}))}async function $e(){if(!je())return R(Le()),D();try{const d=await window.api.listProfileCredentials();if(!d||!d.success||!Array.isArray(d.data))return R(Le()),D();const c=Le();return d.data.forEach(b=>{const I=String(b.profileId||"").toLowerCase();c[I]&&c[I].push({profileId:I,domain:K(b.domain),username:String(b.username||""),password:String(b.password||"")})}),R(c),await Fe(),D()}catch{return R(Le()),D()}}async function xe(d=15e3){if(!je()||Date.now()-ee()<d)return D();if(V())return V();const b=$e().then(I=>(le(Date.now()),I)).finally(()=>{ae(null)});return ae(b),b}function Ue(d){return d?(d.credentialHintsByDomain||(d.credentialHintsByDomain={}),d.credentialHintsByDomain):{}}function Se(d="",c=""){const b=`${String(d||"").toLowerCase()} ${String(c||"").toLowerCase()}`;if(/login|log-in|signin|sign-in|auth|oauth|sso|okta|accounts|session|password|passwd|credential|verify/.test(b))return!0;try{const I=new URL(String(d||""));if(C&&I.origin.toLowerCase()===C){const O=String(I.pathname||"/").toLowerCase(),Y=String(c||"").toLowerCase();if((O==="/"||O==="/login"||O==="/signin")&&Y.includes("synapse"))return!0}const k=`${I.pathname.toLowerCase()} ${I.search.toLowerCase()}`;return/login|signin|auth|sso|oauth|session|password|verify/.test(k)}catch{return!1}}function We(d=""){let c="";try{c=new URL(String(d||"")).hostname.toLowerCase()}catch{return!1}return c==="10.215.56.196"||c.endsWith(".gskinternet.com")||c.endsWith(".gskpro.com")||c.endsWith(".veevavault.com")||c.endsWith(".okta.com")||c.endsWith(".oktacdn.com")||c.endsWith(".pingone.com")}function _e(d="",c="",b=null){return!je()||!b?!1:b.launchedAppType==="website"||!b.launchedAppType?Se(d,c)||We(d):!0}function Xe(d,c=""){if(!d)return null;const b=ze(c);if(b.length===0)return null;const I=D()[d]||[];let k=null,O=-1,Y="";try{const j=K(c);j&&(Y=localStorage.getItem(`oneview:last-used-username:${d}:${j}`)||"")}catch{}return I.forEach(j=>{const a=K(j.domain);if(!a)return;const g=b.reduce((p,y)=>{if(!y)return p;if(y===a)return Math.max(p,1e3);if(me(y)===me(a))return Math.max(p,900);if(y.endsWith(`.${a}`)||a.endsWith(`.${y}`))return Math.max(p,500);const U=me(y),q=me(a);if(U.endsWith(`.${q}`)||q.endsWith(`.${U}`))return Math.max(p,450);const G=U.split(".").reverse(),ce=q.split(".").reverse();let ge=0;for(let de=0;de<Math.min(G.length,ce.length)&&G[de]===ce[de];de+=1)ge+=1;return Math.max(p,ge>1?ge:-1)},-1);if(g<0)return;const m=Y&&String(j.username||"").trim().toLowerCase()===Y.trim().toLowerCase();(!k||g>O||g===O&&m||g===O&&!m&&a.length>K(k.domain).length)&&(k=j,O=g)}),k}function ot(d,c="",b=null){const I=D()[d]||[];if(I.length===0)return null;let k="";try{k=K(c)}catch{k=""}const O=Ue(b),Y=Object.values(O||{}).map(y=>String(y||"").trim()).filter(Boolean),j=String(b?.lastUsernameHint||"").trim()||Y[Y.length-1]||"";if(k&&Ee(k)&&j){const y=Je(d,j,k);if(y)return y}const a=Xe(d,c);if(a&&!Ee(a.domain))return a;if(!j)return null;const g=I.filter(y=>String(y.username||"").trim().toLowerCase()===j.toLowerCase());if(g.length===1)return a&&!Ee(g[0].domain)?a:g[0];if(g.length===0)return null;const m=k;if(!m)return g[0];const p=y=>{const U=K(y);if(!U)return-1;if(Ee(U)&&Pe(m))return-100;if(Ee(m)&&!Ee(U))return 800+(Pe(U)?50:0);if(m===U)return 1e3;if(me(m)===me(U))return 900;if(m.endsWith(`.${U}`)||U.endsWith(`.${m}`))return 500;const q=me(m),G=me(U);if(q.endsWith(`.${G}`)||G.endsWith(`.${q}`))return 450;const ce=q.split(".").reverse(),ge=G.split(".").reverse();let de=0;for(let we=0;we<Math.min(ce.length,ge.length)&&ce[we]===ge[we];we+=1)de+=1;return de};return g.sort((y,U)=>p(U.domain)-p(y.domain)),g[0]||null}function it(d,c="",b=null){const I=ot(d,c,b);if(I)return{...I,profileId:String(I.profileId||"").trim().toLowerCase()||String(d||"").trim().toLowerCase()};const k=D()[d]||[];if(k.length===1)return{...k[0],profileId:String(k[0]?.profileId||"").trim().toLowerCase()||String(d||"").trim().toLowerCase()};let O="";try{O=K(c)}catch{O=""}if(!O||k.length===0)return null;const Y=g=>{const m=K(g);if(!m)return-1;if(Ee(O)&&!Ee(m))return 800+(Pe(m)?50:0);if(Ee(m)&&Pe(O))return-100;if(O===m)return 1e3;if(me(O)===me(m))return 900;if(O.endsWith(`.${m}`)||m.endsWith(`.${O}`))return 500;const p=me(O),y=me(m);if(p.endsWith(`.${y}`)||y.endsWith(`.${p}`))return 450;const U=p.split(".").reverse(),q=y.split(".").reverse();let G=0;for(let ce=0;ce<Math.min(U.length,q.length)&&U[ce]===q[ce];ce+=1)G+=1;return G},a=[...k].sort((g,m)=>Y(m.domain)-Y(g.domain))[0]||null;return a?{...a,profileId:String(a.profileId||"").trim().toLowerCase()||String(d||"").trim().toLowerCase()}:null}async function he(d,c){if(!d||!c)return;const b=String(c.username||""),I=String(c.password||"");if(!I)return!1;const k=`
      (() => {
    ...
    `;try{return await d.executeJavaScript(k,!0),!0}catch{return!1}}function f(d,c){if(!d||!c)return;d._oneviewAutofillTimer&&(clearTimeout(d._oneviewAutofillTimer),d._oneviewAutofillTimer=null);let b=0;const I=async()=>{if(b+=1,!(typeof d.isDestroyed=="function"?d.isDestroyed():!1)&&d.id===`webview-${L()}`){try{if(await d.executeJavaScript("Boolean(window.__oneviewManualCredentialEditAt)",!0)){d._oneviewAutofillTimer&&(clearTimeout(d._oneviewAutofillTimer),d._oneviewAutofillTimer=null);return}}catch{}await he(d,c),b<6&&(d._oneviewAutofillTimer=setTimeout(I,1e3))}};I()}async function T(d){if(d)try{await d.executeJavaScript(`
        (() => {
    ...
    `,w.addEventListener("click",W=>{W.target.classList.contains("tab-close")||G(i.id)}),w.querySelector(".tab-close").addEventListener("click",W=>{W.stopPropagation(),de(i.id)});const $=l.children[s]||null;l.insertBefore(w,$),P()}function m(i){setTimeout(async()=>{const s=E().find(w=>w.id===i);if(!(!s||!s.isHome||document.getElementById(`webview-${i}`)))try{const w=await pe(s);if(!w)return;w.classList.remove("active"),typeof w.hide=="function"&&w.hide().catch(()=>{}),w.syncBounds?.(!1)}catch(w){window.api.webContentCall("log-error",{key:`prewarm-err:${i}:${w.toString()}`}).catch(()=>{})}},0)}async function p(i=null,s=null,l="New Tab",w=null,h={}){const $=h.active!==!1;window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode?s=Ue.gsk:s=gt(s||(Se[ie()]?Se[ie()].partition:Ue.guest));const z=`tab-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,N={id:z,title:l,url:i,partition:s,isHome:!i&&!h.nativePage,nativePage:h.nativePage&&typeof h.nativePage=="object"?t(h.nativePage.type||"settings",h.nativePage.section||"general"):null,lockedProfileId:w,hideBrowserControls:!1,launchedAppType:null,trackingAppId:"",trackingAppName:"",requiresPlatformApi:!1,extensionCompatEnabled:!1,extensionEntryPath:"",extensionActiveContext:{url:"",title:""},credentialAutomationEnabled:!1,lastCredentialSourceProfileId:null};N.trackingAppId=String(h.trackingAppId||"").trim(),N.trackingAppName=String(h.trackingAppName||l||N.title||"").trim(),N.extensionEntryPath=String(h.extensionEntryPath||"").trim(),N.extensionCompatEnabled=!!N.extensionEntryPath,N.extensionActiveContext=h.extensionActiveContext&&typeof h.extensionActiveContext=="object"?{url:String(h.extensionActiveContext.url||"").trim(),title:String(h.extensionActiveContext.title||"").trim()}:{url:"",title:""};const J=E(),De=J.findIndex(st=>st.id===M()),ve=Number.isInteger(h.insertIndex)?Math.max(0,Math.min(h.insertIndex,J.length)):De>=0?De+1:J.length;if(J.splice(ve,0,N),g(N,ve),N.nativePage)await G(z);else if(i){$&&Q(z);const st=document.getElementById("view-home-content"),bt=document.getElementById("webviews-container");$&&st&&st.classList.add("hidden");const Ot=i&&(i.includes("result.html")||i.includes("extension-icon")||i.toLowerCase().includes("result"));if($&&bt){bt.classList.remove("hidden");let Ne=document.getElementById("tab-load-placeholder");Ot?Ne&&(Ne.style.display="none"):Ne?Ne.style.display="flex":(Ne=document.createElement("div"),Ne.id="tab-load-placeholder",Ne.style.cssText=["position:absolute","inset:0","z-index:50","display:flex","flex-direction:column","align-items:center","justify-content:center","background:var(--bg-main,#f8fafc)","gap:16px"].join(";"),Ne.innerHTML=`
          ...
          `,bt.appendChild(Ne));const Lt=()=>{Ne&&(Ne.style.display="none")},pn=Me=>{Me&&(typeof Me._oneviewPlaceholderFinalize=="function"&&(Me.removeEventListener("did-stop-loading",Me._oneviewPlaceholderFinalize),Me.removeEventListener("did-fail-load",Me._oneviewPlaceholderFinalize)),Me._oneviewPlaceholderFinalize=null)},fn=setInterval(()=>{const Me=document.getElementById(`webview-${z}`);if(!Me)return;clearInterval(fn),clearTimeout(mn);const Rt=()=>{pn(Me),clearTimeout(mn),Lt()};pn(Me),Me._oneviewPlaceholderFinalize=Rt,Me.addEventListener("did-stop-loading",Rt,{once:!0}),Me.addEventListener("did-fail-load",Rt,{once:!0})},100),mn=setTimeout(()=>{clearInterval(fn),Lt()},12e3)}await we(z,i,s,l,w,h,$)}else await G(z),ot&&m(z);return N}function y(i=M()){const s=E(),l=s.find(z=>z.id===i);if(!l)return;const w=s.findIndex(z=>z.id===l.id),h=document.getElementById(`webview-${l.id}`),$=h&&typeof h.getURL=="function"&&h.getURL()||l.url||"",W=h&&typeof h.getTitle=="function"&&h.getTitle()||l.title||"New Tab";p(l.isHome||!$||$==="about:blank"?null:$,l.partition,W,l.lockedProfileId||null,{insertIndex:w>=0?w+1:s.length,trackingAppId:l.trackingAppId||"",trackingAppName:l.trackingAppName||W})}function U(i){const s=E();if(!s.length)return;const l=s.findIndex($=>$.id===M()),h=((l>=0?l:0)+i+s.length)%s.length;G(s[h].id,{suppressHomeSearchFocus:!0})}async function q(i){!i||typeof i.focusWebContents!="function"||await i.focusWebContents().catch(()=>{})}async function G(i,s={}){Q(i);const l=E().find(N=>N.id===i);if(!l)return;v(),x().catch(()=>{}),S(l),C(l),document.querySelectorAll(".tab").forEach(N=>N.classList.remove("active"));const w=document.getElementById(`tab-ui-${i}`);w&&w.classList.add("active");const h=document.getElementById("view-home-content"),$=document.getElementById("nativeTabContent"),W=document.getElementById("webviews-container"),z=document.querySelectorAll(".webviews-container .webcontent-pane");if(l.isHome){h.classList.remove("hidden"),$?.classList.add("hidden"),W.classList.add("hidden"),Z(l),oe(l),d(l);const N=document.getElementById("googleSearchInput");N&&s?.suppressHomeSearchFocus!==!0&&(N.value="",N.focus())}else if(l.nativePage)h.classList.add("hidden"),$?.classList.remove("hidden"),W.classList.add("hidden"),Z(l),oe(l),d(l),k(""),r(l.nativePage?.section)==="general"&&await o(),u(l);else{h.classList.add("hidden"),$?.classList.add("hidden"),W.classList.remove("hidden"),Z(l),oe(l),d(l),z.forEach(J=>{J.id!==`webview-${i}`&&(J.classList.remove("active"),typeof J.hide=="function"&&J.hide().catch(()=>{}),J.syncBounds?.(!1))});let N=null;try{N=document.getElementById(`webview-${i}`),N||(N=await pe(l))}catch(J){window.api.webContentCall("log-error",{key:`switchTab-create-err:${i}:${J.toString()}`}).catch(()=>{})}if(N)try{N.classList.add("active"),typeof N.show=="function"&&await N.show().catch(()=>{}),N.syncBounds?.(!0),await q(N),k(N.getURL())}catch(J){window.api.webContentCall("log-error",{key:`switchTab-show-err:${i}:${J.toString()}`}).catch(()=>{})}}a(l.id),B(),Y()}function ce(i){if(!i)return!1;const s=String(i.launchedAppType||"").toLowerCase();return We.has(s)}function ge(i,s){if(!i||!s||!ce(s))return!1;const l=String(i.getAttribute("partition")||s.partition||"");if(!l)return!1;const w=i.parentElement;for(w&&w.removeChild(i),i.classList.remove("active"),he.push({webview:i,partition:l,platformApiEnabled:R(i),at:Date.now()}),L("view-webcontent","park-to-pool",{partition:l,poolSize:he.length});he.length>_e;){const h=he.shift();h&&h.webview&&!h.webview.isDestroyed?.()&&h.webview.remove()}return!0}function de(i){const s=E(),l=s.findIndex(W=>W.id===i);if(l===-1)return;const w=M()===i;document.getElementById(`tab-ui-${i}`)?.remove();const h=document.getElementById(`webview-${i}`);if(h&&(j(h),(!ce(s[l])||!ge(h,s[l]))&&h.remove()),s.splice(l,1),s.length===0){Q(null),p(),P();return}const $=s.some(W=>W.id===M());if(w||!$){const W=Math.min(l,s.length-1),z=s[W]||s[s.length-1];z&&G(z.id)}P()}async function we(i,s,l,w,h=null,$={},W=!0){const z=performance.now(),N=E().find(De=>De.id===i);if(!N)return;N._navStartedAt=z,L("view-nav","navigateTo-start",{tabId:N.id,url:String(s||""),partition:String(l||""),appType:$.appType||null}),N.isHome=!1,N.url=s,N.partition=l,N.lockedProfileId=h,N.hideBrowserControls=!!$.hideControls,N.launchedAppType=$.appType||null,N.lastCredentialSourceProfileId=null,N.trackingAppId=String($.trackingAppId||"").trim(),N.trackingAppName=String($.trackingAppName||w||N.title||"").trim(),N.requiresPlatformApi=D(s,$),w&&c(i,w),W&&(Q(i),await G(i));let J=document.getElementById(`webview-${i}`);if(!J)J=await pe(N,W);else{const De=J.getAttribute("partition"),ve=R(J);(De!==l||ve!==!!N.requiresPlatformApi)&&(j(J),(!ce(N)||!ge(J,N))&&J.remove(),J=await pe(N,W))}W&&typeof J.show=="function"?await J.show().catch(()=>{}):!W&&typeof J.hide=="function"&&await J.hide().catch(()=>{}),typeof J.setMeta=="function"&&await J.setMeta({trackingAppId:N.trackingAppId,appName:N.trackingAppName,appType:N.launchedAppType||""}).catch(()=>{}),J.syncBounds?.(W),W&&(k(s),await q(J)),J.src!==s&&(J.src=s),L("view-nav","navigateTo-dispatch",{tabId:N.id,elapsedMs:Math.round(performance.now()-z)})}async function te(i,s,l,w=null,h={}){return we(M(),i,s,l,w,h,!0)}function ne(i){i&&i.executeJavaScript(`
      (() => {
  ...
  `;try{i.insertCSS(l)}catch(w){console.warn("Could not apply website scrollbar theme:",w)}}function wi(){const i=E().find(l=>l.id===M());if(!i||i.isHome)return;i.isHome=!0,i.url=null,i.lockedProfileId=null,c(i.id,"New Tab");const s=document.getElementById(`webview-${i.id}`);s&&(j(s),(!ce(i)||!ge(s,i))&&s.remove()),G(i.id)}function hi(i=""){const s=String(i||"").trim();if(!s)return"";const l=s.match(/^https?:\/\/([a-zA-Z])(?:\/|%2[fF]|\\)(.*)$/);if(l){const h=l[1].toUpperCase(),$=decodeURIComponent(l[2]).replace(/\\/g,"/").replace(/^\/+/,"");return`file:///${h}:/${$}`}if(/^file:\/\//i.test(s)||/^[a-z][a-z0-9+.-]*:\/\//i.test(s)||/^about:/i.test(s))return s;const w=s.match(/^([a-zA-Z])[:/\\](.*)$/);if(w){const h=w[1].toUpperCase(),$=w[2].replace(/\\/g,"/").replace(/^\/+/,"");return`file:///${h}:/${$}`}if(/^\/[A-Za-z]\//.test(s)){const h=s[1].toUpperCase(),$=s.slice(3).replace(/\\/g,"/");return`file:///${h}:/${$}`}return/^\\\\/.test(s)?`file:${s.replace(/\\/g,"/")}`:s}function Nt(i=""){const s=String(i||"").trim();return!s||/\s/.test(s)?!1:!!(/^about:/i.test(s)||/^[a-z][a-z0-9+.-]*:\/\//i.test(s)||/^localhost(?::\d+)?(?:[/?#].*)?$/i.test(s)||/^\d{1,3}(?:\.\d{1,3}){3}(?::\d+)?(?:[/?#].*)?$/.test(s)||s.includes(".")||/[/:?#]/.test(s))}async function vi(i){if(i=hi(i),!i)return;if(window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode){let h=i;Nt(i)?!/^[a-z][a-z0-9+.-]*:\/\//i.test(i)&&!/^about:/i.test(i)&&(h=`https://${i}`):h=`https://www.google.com/search?q=${encodeURIComponent(i)}`,await te(h,Ue.gsk,i);return}let s=i,l=Nt(i);if(l?!/^[a-z][a-z0-9+.-]*:\/\//i.test(i)&&!/^about:/i.test(i)&&!/^file:\/\//i.test(i)&&(s=`https://${i}`):Nt(i)?(s=`https://${i}`,l=!0):s=`https://www.google.com/search?q=${encodeURIComponent(i)}`,l){const h=await Pe(s,null,"New Tab");if(!h||h.cancelled)return;const $=h.profileId,W=h.partition;$&&$!==ie()&&qe($,{bypassLock:!0}),await te(s,W,"New Tab",h.lockedProfileId);return}const w=Je(s,b()?.partition||Se[ie()]?.partition||Ue.guest,"Google Search");await te(s,w,"Google Search")}function bi(i,{isTeardown:s=!1}={}){const l=E(),w=Array.isArray(i)?i.filter(Boolean):[];if(w.length===0||l.length===0)return;const h=new Set(w),$=[...l],W=$.findIndex(z=>z.id===M());if($.forEach(z=>{if(!h.has(z.id))return;document.getElementById(`tab-ui-${z.id}`)?.remove();const N=document.getElementById(`webview-${z.id}`);N&&(j(N),(!ce(z)||!ge(N,z))&&N.remove())}),A($.filter(z=>!h.has(z.id))),E().length===0){Q(null),s||(p(),P());return}if(h.has(M())){const z=Math.min(Math.max(W,0),E().length-1);Q(E()[z].id)}G(M()||E()[0].id),P()}function cn(i="",s="",l=null){return l?l.launchedAppType==="website"||!l.launchedAppType?dn(i,s)||un(i):!0:!1}function dn(i="",s=""){const l=`${String(i||"")} ${String(s||"")}`.toLowerCase();if(/login|sign in|signin|password|sso|authenticate|verify/.test(l))return!0;try{const w=new URL(String(i||"")),h=`${w.pathname.toLowerCase()} ${w.search.toLowerCase()}`;return/login|signin|auth|sso|oauth|session|password|verify/.test(h)}catch{return!1}}function un(i=""){let s="";try{s=new URL(String(i||"")).hostname.toLowerCase()}catch{return!1}return s==="10.215.56.196"||s.endsWith(".gskinternet.com")||s.endsWith(".gskpro.com")||s.endsWith(".veevavault.com")||s.endsWith(".okta.com")||s.endsWith(".oktacdn.com")||s.endsWith(".pingone.com")}return{closeTab:de,closeTabsBulk:bi,createTab:p,createWebviewForTab:pe,duplicateTab:y,disposeWebviewRuntime:fi,getActiveTab:b,getActiveWebview:I,goToActiveTabHome:wi,isInspectableLocalFileTab:mi,navigateTo:te,navigateToTab:we,performSearch:vi,startWebviewRuntime:Ut,switchRelativeTab:U,switchTab:G,updateTabTitle:c,updateUrlDisplay:k,updateUrlDisplayString:O,getCurrentProfileId:ie,getTabs:E}}console.log("View Page Script initializing...");let re=[],Ve=null,Tt=!1,Rn=null,hn=null,vn=[],bn=[],yn={wppproduction:[],vml:[],gsk:[],guest:[],synapse:[],contentgen:[]};const Gt=et.profileHistory,Jt="view.profileHistory.v1";let Te={wppproduction:[],vml:[],gsk:[],guest:[]};const Fn=et.customBookmarks;let Oe=[],Sn="guest",zt="guest",En="",dt="guest",Pt=null,Qe=null,xn=null;const Ti=new Set(["login.veevavault.com","federation.gsk.com"]);let yt=null,Re=-1,He=[],Cn={version:"",defaultOpenStatus:""},In=0,Ln=null,kn=!1;const Wn=et.perfEnabled;let pt=null,Pn=!1;const It={};Object.defineProperties(It,{tabs:{get:()=>re,set:e=>{re=e}},activeTabId:{get:()=>Ve,set:e=>{Ve=e}},browserExtensionsCache:{get:()=>vn,set:e=>{vn=e}},managedDownloadsCache:{get:()=>bn,set:e=>{bn=e}},nativeSettingsGeneralInfo:{get:()=>Cn,set:e=>{Cn=e}},historyProfileId:{get:()=>zt,set:e=>{zt=e}},historySearchQuery:{get:()=>En,set:e=>{En=e}},currentProfileId:{get:()=>fe,set:e=>{fe=e}},profileHistoryCache:{get:()=>Te,set:e=>{Te=e}},credentialCache:{get:()=>yn,set:e=>{yn=e}},passwordProfileId:{get:()=>Sn,set:e=>{Sn=e}},passwordEditTarget:{get:()=>hn,set:e=>{hn=e}},activeHttpAuthChallenge:{get:()=>xn,set:e=>{xn=e}},credentialCacheRefreshedAt:{get:()=>In,set:e=>{In=e}},credentialCacheRefreshInFlight:{get:()=>Ln,set:e=>{Ln=e}}});const qt={synapse:{partition:be.synapse},contentgen:{partition:be.contentgen}},Xt=new Set(["nextjs","vite","react","angular","html","neutralino","website","vite-server"]),Hn=(()=>{try{return new URL(Ii).origin.toLowerCase()}catch{return""}})(),$i=new URL(""+new URL("contentgen-DHdropzE.webp",import.meta.url).href,import.meta.url).href,_i=new URL(""+new URL("contentgen-dark-Bj2lxwBn.webp",import.meta.url).href,import.meta.url).href;function Bn(e=document){if(!e||typeof e.querySelectorAll!="function")return;const n=document.body.classList.contains("dark-mode");e.querySelectorAll("img[data-theme-icon]").forEach(t=>{const o=String(t.getAttribute("data-theme-icon")||"").trim();let r="";o==="contentgen"&&(r=n?_i:$i),r&&t.getAttribute("src")!==r&&t.setAttribute("src",r)})}function Di(){["httpAuthModal","bookmarkModal","cacheActionModal","extensionsManagerModal"].forEach(e=>{const n=document.getElementById(e);!n||n.dataset.hoistedToBody==="1"||(document.body.appendChild(n),n.dataset.hoistedToBody="1")})}async function An(){const e=tt();if(!(!e||e.isHome||!e.url||!window.api?.openDetachedViewWindow))try{await window.api.openDetachedViewWindow({url:e.url,title:e.title||"Detached Tab",partition:e.partition||be.guest}),Zt(e.id)}catch(n){console.error("Failed to open detached tab window",n),Ke("Could not open the page in a separate window.","error")}}function Mt(){if(pt!==null)return pt;try{const e=localStorage.getItem(Wn);return pt=e==="1"||e==="true",pt}catch{return pt=!1,!1}}function Vn(e,n,t=null){if(!Mt())return;const o=t?{...t}:{};try{console.log(`[PERF][${e}] ${n}`,o)}catch{}}window.addEventListener("storage",e=>{e.key===Wn&&(pt=null,window.api&&typeof window.api.setPerfLoggingEnabled=="function"&&window.api.setPerfLoggingEnabled(Mt()).catch(()=>{}))});const se={wppproduction:{id:"wppproduction",name:"WPPProduction",partition:be.wppproduction,color:"#000000",bgColor:"#e2e8f0",label:"W"},vml:{id:"vml",name:"VML",partition:be.vml,color:"#ff0000",bgColor:"#fee2e2",label:"V"},gsk:{id:"gsk",name:"GSK",partition:be.gsk,color:"#f37521",bgColor:"#ffedd5",label:"G"},guest:{id:"guest",name:"Guest",partition:be.guest,color:"#64748b",bgColor:"#f1f5f9",label:"?"}};let fe="guest";function Mi(){return fe}const Ui=Bi({state:It,constants:{AUTH_GATEWAY_HOSTS:Ti,RESOURCE_SERVICE_ORIGIN:Hn,PROFILES:se},showToast:Ke,openOverlayModal:lt,closeOverlayModal:Ze,renderProfilePillSelect:on,resolveCredentialScopeIdForTab:ii,applyProfileSelection:ht,getCurrentProfileId:()=>fe,escapeHtml:Ce}),{getAutofillCredentialForTab:Ni,initHttpAuthPrompt:Oi,installCredentialCaptureHooks:Ri,isLikelyAuthPage:Fi,maybeOfferRememberCredentials:Wi,refreshCredentialCacheIfStale:Hi,scheduleCredentialAutofill:Vi,startCredentialCapturePolling:ji,handleCredentialFieldInteraction:zi,hideCredentialDropdown:qi}=Ui;let ut=null,St=null;const Yi=Pi({state:It,constants:{PROFILES:se,PENDING_EXTENSION_OPEN_STORAGE_KEY:"oneview.pendingExtensionOpenPath",EXTENSION_PIN_STORAGE_KEY:"oneview.browserExtensions.pinned.v1",IS_DEV_APP_BUILD:At},escapeHtml:Ce,showToast:Ke,openOverlayModal:lt,closeOverlayModal:Ze,getActiveTab:(...e)=>St?.getActiveTab?.(...e)??null,getActiveWebview:(...e)=>St?.getActiveWebview?.(...e)??null,createTab:(...e)=>St?.createTab?.(...e),refreshActiveNativeSettingsPage:(...e)=>ut?.refreshActiveNativeSettingsPage?.(...e)??Promise.resolve(),closeSettingsMenu:xo}),{closeBrowserExtensionPopup:Qt,closeBrowserExtensionsMenu:Ki,formatDownloadBytes:Gi,formatDownloadEta:Ji,formatDownloadSpeed:Xi,initDownloadsManager:Qi,initExtensionsManager:Zi,loadManagedDownloadsFromMain:eo,refreshBrowserExtensionsUi:to,refreshExtensionsManagerList:jn}=Yi;St=Ai({state:It,constants:{PARTITIONS:be,PROFILES:se,LOCAL_WEB_APP_TYPES:Xt,WEBVIEW_POOL_MAX:6,WEBVIEW_POOL_KEEPALIVE_MS:12e3,TAB_PREWARM_ENABLED:!0,PREWARM_ALL_PROFILE_PARTITIONS:!1},createNativePageDescriptor:(...e)=>ut?.createNativePageDescriptor?.(...e)??null,ensureNativeSettingsGeneralInfo:(...e)=>ut?.ensureNativeSettingsGeneralInfo?.(...e)??Promise.resolve(),normalizeSettingsSection:(...e)=>ut?.normalizeSettingsSection?.(...e)??"general",renderNativeSettingsPage:(...e)=>ut?.renderNativeSettingsPage?.(...e),closeBrowserExtensionsMenu:Ki,closeBrowserExtensionPopup:Qt,refreshBrowserExtensionsUi:to,refreshTabScrollControls:Dt,syncProfileSelectionForTab:Po,updateProfileLockUI:Et,perfMark:Vn,shouldRequirePlatformApiForNavigation:Do,getWebviewPlatformApiFlag:Mo,setWebviewPlatformApiFlag:Uo,getWebviewPreloadPathCached:_o,startCredentialCapturePolling:ji,scheduleCredentialAutofill:Vi,installCredentialCaptureHooks:Ri,refreshCredentialCacheIfStale:Hi,getAutofillCredentialForTab:Ni,resolveAssignedProfileIdForTab:Jn,getCredentialScopeIdByPartition:qn,resolveCredentialScopeIdForTab:ii,maybeOfferRememberCredentials:Wi,syncTabProfileForPage:Bo,trackProfileHistory:$o,resolveProfileIdForTab:ni,resolveAssignedProfileId:Ct,resolveStrictProfileNavigationTarget:Xn,resolveNavigationPartition:ft,applyProfileSelection:ht,openProfilePromptDialog:Lo,handleCredentialFieldInteraction:zi,hideCredentialDropdown:qi});const{closeTab:Zt,closeTabsBulk:mt,createTab:Ae,createWebviewForTab:no,duplicateTab:io,disposeWebviewRuntime:zn,getActiveTab:tt,getActiveWebview:Ge,goToActiveTabHome:oo,isInspectableLocalFileTab:ro,navigateTo:xt,navigateToTab:so,performSearch:$t,startWebviewRuntime:ao,switchRelativeTab:kt,switchTab:Ye,updateTabTitle:lo,updateUrlDisplay:co,updateUrlDisplayString:Tn,getTabs:Yt}=St;ut=ki({state:It,constants:{PROFILES:se,PARTITIONS:be,IS_DEV_APP_BUILD:At},escapeHtml:Ce,showToast:Ke,isPerfEnabled:Mt,formatDownloadBytes:Gi,formatDownloadSpeed:Xi,formatDownloadEta:Ji,formatHistoryTime:oi,loadManagedDownloadsFromMain:eo,refreshExtensionsManagerList:jn,saveProfileHistoryStore:rn,getActiveTab:tt,updateTabTitle:lo,createTab:Ae,switchTab:Ye,navigateTo:xt});const{bindSettingsShortcutsGlobal:uo,createNativePageDescriptor:mr,ensureNativeSettingsGeneralInfo:gr,getSettingsTabTitle:wr,initNativeSettingsUi:po,initSettingsMenu:fo,isEditableShortcutTarget:mo,isNativeSettingsTab:hr,normalizeSettingsSection:vr,openSettingsTab:Kt,refreshActiveNativeSettingsPage:go,renderNativeSettingsPage:br,updateNativeSettingsTabSection:yr}=ut;function wo(){try{return localStorage.getItem("username")==="Guest"}catch{return!1}}function ho(e="",n="",t=""){const o=String(t||"").trim().toLowerCase();if(qt[o])return o;const r=String(n||"").trim().toLowerCase(),u=String(e||"").trim().toLowerCase();try{const v=new URL(String(e||"")),x=`${v.protocol}//${v.host}`.toLowerCase(),B=String(v.pathname||"/").toLowerCase();if(x===Hn&&(B==="/"||B===""))return"synapse";if(x==="http://10.215.56.196:3456")return"contentgen"}catch{}return/content[\s-]*gen/.test(r)||/content[\s-]*gen/.test(u)?"contentgen":r.includes("synapse")?"synapse":""}function en(e){const n=gt(e);if(n===be.synapse||n===be.contentgen)return"guest";const t=Object.values(se).find(o=>o.partition===n);return t?t.id:null}function qn(e){const n=gt(e);return n===be.synapse?"synapse":n===be.contentgen?"contentgen":en(n)}function ft(e="",n="",t="",o={}){const r=String(o?.sessionScope||o?.credentialScope||"").trim()||"",u=ho(e,t,r);return u&&qt[u]?qt[u].partition:gt(n||se[fe]?.partition||be.guest)}async function vo(){const e=document.getElementById("view-synapse-link-btn");if(!e||(e.style.display="none",wo()))return;const n=String(localStorage.getItem("emp_id")||"").trim(),t=String(localStorage.getItem("username")||"").trim(),o=/^\d+$/.test(t)?t:"",r=n||o;if(r)try{const{data:u}=await Ci("resources",{},{limit:200,timeoutMs:5e3}),v=new Set(u.map(x=>String(x?.emp_id??"").trim()).filter(Boolean));e.style.display=v.has(r)?"flex":"none"}catch(u){console.warn("Could not verify Synapse visibility in view",u),e.style.display="none"}}function bo(e="",n="addressSearchDropdown"){const t=document.getElementById(n);if(!t)return;const o=String(e||"").trim().toLowerCase();if(!o){t.classList.add("hidden"),He=[],Re=-1;return}const r=re.filter(S=>{const C=String(S.title||"").toLowerCase(),L=String(S.url||"").toLowerCase();return C.includes(o)||L.includes(o)}).map(S=>({type:"tab",id:S.id,title:S.title||"Untitled Tab",url:S.url||"",partition:S.partition}));let u=[];Object.values(Te).forEach(S=>{Array.isArray(S)&&S.forEach(C=>{const L=String(C.title||"").toLowerCase(),D=String(C.url||"").toLowerCase();if(L.includes(o)||D.includes(o)){const R=u.some(H=>H.url===C.url),F=r.some(H=>H.url===C.url);!R&&!F&&u.push({type:"history",title:C.title||"History Item",url:C.url,partition:C.partition||be.guest})}})}),He=[{type:"search",title:e,url:`Search for "${e}"`},...r.slice(0,5),...u.slice(0,15)],Re=0;let x="";x+=Ft(He[0],0);const B=He.filter(S=>S.type==="tab");B.length>0&&(x+='<div class="address-search-group-label">Open Tabs</div>',B.forEach(S=>{const C=He.indexOf(S);x+=Ft(S,C)}));const P=He.filter(S=>S.type==="history");P.length>0&&(x+='<div class="address-search-group-label">History</div>',P.forEach(S=>{const C=He.indexOf(S);x+=Ft(S,C)})),t.innerHTML=x,t.classList.remove("hidden")}function Ft(e,n){const t=n===Re;let o=String(e.title||"H").charAt(0).toUpperCase(),r="is-history",u="History";return e.type==="tab"?(r="is-tab",u="Tab"):e.type==="search"&&(r="is-search",u="Search",o="🔍"),`
  ...
  `}async function $n(e,n){const t=document.getElementById(e);if(!t)return;if(Re<0||Re>=He.length){$t(t.value);return}const o=He[Re],r=document.getElementById(n);r&&r.classList.add("hidden"),o.type==="tab"?await Ye(o.id):o.type==="history"?await ri(o.url,o.partition,o.title):$t(t.value),t.blur()}function _n(e,n){const t=document.getElementById(e),o=document.getElementById(n);!t||!o||(t.addEventListener("input",()=>{bo(t.value,n)}),t.addEventListener("keydown",r=>{o.classList.contains("hidden")||(r.key==="ArrowDown"?(r.preventDefault(),Re=(Re+1)%He.length,Dn(n)):r.key==="ArrowUp"?(r.preventDefault(),Re=(Re-1+He.length)%He.length,Dn(n)):r.key==="Enter"?(r.preventDefault(),$n(e,n)):r.key==="Escape"&&o.classList.add("hidden"))}),o.addEventListener("click",r=>{const u=r.target.closest(".address-search-item");if(!u)return;const v=parseInt(u.dataset.index);isNaN(v)||(Re=v,$n(e,n))}))}function Dn(e){const n=document.getElementById(e);n&&n.querySelectorAll(".address-search-item").forEach((t,o)=>{const r=parseInt(t.dataset.index);t.classList.toggle("is-selected",r===Re),r===Re&&t.scrollIntoView({block:"nearest"})})}function yo(){const e=document.getElementById("googleSearchInput");e&&e.dataset.boundAddressInput!=="1"&&(e.dataset.boundAddressInput="1",e.addEventListener("keydown",t=>{const o=document.getElementById("addressSearchDropdownHome");o&&!o.classList.contains("hidden")||t.key==="Enter"&&$t(e.value)}),_n("googleSearchInput","addressSearchDropdownHome"));const n=document.getElementById("urlDisplay");n&&n.dataset.boundAddressInput!=="1"&&(n.dataset.boundAddressInput="1",n.addEventListener("focus",()=>{n.select?.()}),n.addEventListener("keydown",t=>{const o=document.getElementById("addressSearchDropdown");if(!(o&&!o.classList.contains("hidden"))){if(t.key==="Enter")t.preventDefault(),$t(n.value),n.blur();else if(t.key==="Escape"){t.preventDefault();const r=tt();co(r?.url||""),n.blur()}}}),_n("urlDisplay","addressSearchDropdown"))}function Yn(){if(Tt){window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode?window.enterSiteSnapStudioMode():window.exitSiteSnapStudioMode(),re.length===0?Ae():Ye(Ve||re[0].id);return}Tt=!0,console.log("initViewPage called"),yi().catch(n=>{console.warn("Could not initialize OneView shared storage sync",n)}),or(),window.api&&typeof window.api.setPerfLoggingEnabled=="function"&&window.api.setPerfLoggingEnabled(Mt()).catch(()=>{}),window.api&&typeof window.api.getPerfLogPath=="function"&&window.api.getPerfLogPath().then(n=>{n?.success&&Vn("perf","log-path",{path:n.path||"",enabled:n.enabled})}).catch(()=>{}),!kn&&window.api&&typeof window.api.onCredentialDebugLog=="function"&&(kn=!0,window.api.onCredentialDebugLog(n=>{console.log("[OneView][CredentialCapture][MainRelay]",n)})),document.body?.dataset.boundProfileHistorySharedStorage!=="1"&&window.api?.onOneviewSharedStorageUpdated&&(document.body.dataset.boundProfileHistorySharedStorage="1",window.api.onOneviewSharedStorageUpdated(n=>{String(n?.key||"")===Jt&&(ti(n),go().catch(()=>{}))})),Wo(),To(),Ao().catch(()=>{}),zo(),sn(),Bn(),Di(),document.addEventListener("click",n=>{const t=document.getElementById("addressSearchDropdown"),o=document.getElementById("addressSearchDropdownHome"),r=document.getElementById("urlDisplay"),u=document.getElementById("googleSearchInput");t&&!t.contains(n.target)&&n.target!==r&&t.classList.add("hidden"),o&&!o.contains(n.target)&&n.target!==u&&o.classList.add("hidden");const v=document.getElementById("credentialSelectionDropdown");v&&!v.contains(n.target)&&v.classList.add("hidden")}),document.body.dataset.viewThemeIconObserverBound||(document.body.dataset.viewThemeIconObserverBound="1",new MutationObserver(()=>{Bn()}).observe(document.body,{attributes:!0,attributeFilter:["class"]})),re.length===0?Ae():Ye(Ve||re[0].id);const e=document.getElementById("newTabBtn");e&&e.addEventListener("click",()=>{Ae()}),Go();try{yo()}catch(n){window.api.webContentCall("log-error",{key:`viewJS-setupViewSearch-err:${n.toString()}`}).catch(()=>{})}try{Ho()}catch(n){window.api.webContentCall("log-error",{key:`viewJS-setupBookmarks-err:${n.toString()}`}).catch(()=>{})}vo().catch(n=>{console.warn("Failed to update Synapse visibility in view",n)});try{Yo()}catch(n){window.api.webContentCall("log-error",{key:`viewJS-initBookmarkManager-err:${n.toString()}`}).catch(()=>{})}document.getElementById("browserBack")?.addEventListener("click",()=>{const n=Ge();if(n&&n.canGoBack()){n.goBack();return}oo()}),document.getElementById("browserForward")?.addEventListener("click",()=>{const n=Ge();n&&n.canGoForward()&&n.goForward()}),document.getElementById("browserReload")?.addEventListener("click",()=>{const n=Ge();n&&n.reload()}),document.getElementById("browserDetach")?.addEventListener("click",An),document.getElementById("browserDetachHeader")?.addEventListener("click",An),Eo(),ao(),Xo(),Oi(),ko(),fo(),uo(),po(),Zi(),Qi(),window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode?window.enterSiteSnapStudioMode():window.exitSiteSnapStudioMode()}window.addEventListener("beforeunload",()=>{if(Qt().catch(()=>{}),zn(),typeof re<"u"&&Array.isArray(re)){const e=re.map(n=>n.id);mt(e,{isTeardown:!0})}});window.addEventListener("teardown-view-system",()=>{console.log("Teardown View System triggered"),Qt().catch(()=>{}),zn();const e=re.map(t=>t.id);mt(e,{isTeardown:!0});const n=document.getElementById("webviews-container");n&&(n.querySelectorAll(".webcontent-pane").forEach(o=>{try{typeof o.remove=="function"&&o.remove()}catch{}}),n.innerHTML=""),re=[],Ve=null,Tt=!1});function Wt(e){if(!e||e.isHome)return!1;const n=String(e.url||"").trim(),t=String(e.launchedAppType||"").trim().toLowerCase();return!n||n==="about:blank"||n==="newtab"||Bt(n)||!/^https?:\/\//i.test(n)?!1:!t||t==="website"}function So({activateVisibleTab:e=!0}={}){if(!Tt||re.length===0)return;const n=re.filter(u=>!u.isHome&&!Wt(u)).map(u=>u.id);if(n.length>0&&mt(n),!e)return;const t=tt();if(t&&Wt(t)){Ye(t.id);return}const o=re.find(u=>Wt(u));if(o){Ye(o.id);return}const r=re.find(u=>u.isHome);if(r){Ye(r.id);return}if(re.length===0){Ae();return}Ye(re[0].id)}window.addEventListener("ticket-switch-preserve-view",e=>{So({activateVisibleTab:e?.detail?.activateVisibleTab!==!1})});async function Eo(){if(!window.api||typeof window.api.resolveOneviewAppUrl!="function")return;const e=JSON.parse(localStorage.getItem(et.installedApps)||"{}");let n=!1;for(const[t,o]of Object.entries(e)){const r=String(o?.type||"").toLowerCase();if(Xt.has(r)&&o?.localPath)try{const u=await window.api.resolveOneviewAppUrl(t,o.localPath);u?.success&&u.url&&o.oneviewUrl!==u.url&&(e[t]={...o,oneviewUrl:u.url},n=!0)}catch{}}n&&localStorage.setItem(et.installedApps,JSON.stringify(e))}window.initViewPage=Yn;window.closeViewTab=Zt;function xo(){const e=document.getElementById("settingsBtn");e&&e.classList.remove("is-active")}function tn(){return{modal:document.getElementById("extensionPromptModal"),title:document.getElementById("extensionPromptTitle"),message:document.getElementById("extensionPromptMessage"),label:document.getElementById("extensionPromptLabel"),input:document.getElementById("extensionPromptInput"),textarea:document.getElementById("extensionPromptTextarea"),form:document.getElementById("extensionPromptForm"),submitBtn:document.getElementById("extensionPromptSubmitBtn"),cancelBtn:document.getElementById("extensionPromptCancelBtn"),closeBtn:document.getElementById("extensionPromptCloseBtn")}}async function Co(e={}){const n=tn();if(!n.modal||!n.form||!n.input||!n.textarea)return{cancelled:!0,value:""};if(Qe)return{cancelled:!0,value:""};const t=e&&typeof e=="object"?e:{},o=t.multiline===!0,r=t.required!==!1,u=String(t.value||""),v=String(t.title||"Extension Input").trim()||"Extension Input",x=String(t.message||"").trim(),B=String(t.label||"Value").trim()||"Value",P=String(t.submitLabel||"Submit").trim()||"Submit",S=String(t.cancelLabel||"Cancel").trim()||"Cancel",C=String(t.placeholder||"").trim();return n.title.textContent=v,n.message.textContent=x,n.message.classList.toggle("hidden",!x),n.label.textContent=B,n.submitBtn.textContent=P,n.cancelBtn.textContent=S,n.input.classList.toggle("hidden",o),n.textarea.classList.toggle("hidden",!o),n.input.required=!o&&r,n.textarea.required=o&&r,n.input.type=t.password===!0?"password":"text",n.input.placeholder=C,n.textarea.placeholder=C,n.input.value=o?"":u,n.textarea.value=o?u:"",new Promise(L=>{Qe={resolve:L,required:r,multiline:o},lt(()=>{n.modal.classList.remove("hidden"),n.modal.setAttribute("aria-hidden","false"),requestAnimationFrame(()=>{(o?n.textarea:n.input).focus(),(o?n.textarea:n.input).select?.()})}).catch(()=>{Qe=null,L({cancelled:!0,value:""})})})}function Mn(e={cancelled:!0,value:""}){const n=tn();if(!n.modal||!Qe)return;const t=Qe;Qe=null,Ze(()=>{n.modal.classList.add("hidden"),n.modal.setAttribute("aria-hidden","true"),n.form.reset(),n.input.classList.remove("hidden"),n.textarea.classList.add("hidden"),n.input.type="text"}),t.resolve(e)}function Io(){return{modal:document.getElementById("profilePromptModal"),select:document.getElementById("profilePromptSelect"),continueBtn:document.getElementById("profilePromptContinueBtn"),cancelBtn:document.getElementById("profilePromptCancelBtn"),closeBtn:document.getElementById("profilePromptCloseBtn")}}let Ht=null;async function Lo(e,n="guest"){const t=Io();if(!t.modal||!t.select)return{cancelled:!0,profileId:n};if(Ht)return{cancelled:!0,profileId:n};let o=n;const r=t.select;r.innerHTML=Object.values(se).map(v=>{const x=String(v.name||"P").charAt(0).toUpperCase();return`
    ...
    `,document.body.appendChild(n));const t=n.querySelector("p");return t&&(t.textContent=`Opening ${e||"app"}...`),()=>{n&&(n.style.display="none")}}async function Fo(e){if(!e)return;const t=JSON.parse(localStorage.getItem(et.installedApps)||"{}")[e];if(!t){console.warn("Installed app not found:",e);return}const o=t.name||"App",r=Ro(o),u=Si(t);console.log("[OneView Tracking] View launch decision",{appId:e,appName:t?.name||"",appType:t?.type||"",clickTrackingMode:t?.clickTrackingMode||"",trackOnLaunch:t?.trackOnLaunch,oneviewUrl:t?.oneviewUrl||"",hasLocalPath:!!t?.localPath,shouldTrackLaunch:u}),u&&Ei({currentSelectedTicketId:window.currentActiveTicketKey||"",clickedAppName:o});try{if(wn(t)){await xi(t),Ke(`Opened "${o}" in a separate window.`,"info");return}const v=()=>tt()?.partition||se[fe]?.partition||be.guest,x=async(P,S={})=>{await Ae(P,v(),o,null,{hideControls:!0,appType:t.type,trackingAppId:e,trackingAppName:o,bypassPrompt:!0,...S})},B=String(t.type||"").toLowerCase();if(Xt.has(B)&&t.localPath){Tn(`Starting ${o}...`);let P=String(t.oneviewUrl||"").trim();if(window.api&&typeof window.api.resolveOneviewAppUrl=="function"){const C=await window.api.resolveOneviewAppUrl(e,t.localPath,"/",v());if(C?.success&&C.url){P=C.url;const L=JSON.parse(localStorage.getItem(et.installedApps)||"{}");L[e]&&(L[e]={...L[e],oneviewUrl:P},localStorage.setItem(et.installedApps,JSON.stringify(L)))}}const S=["nextjs","next","vite-server"].includes(B);if(P&&Bt(P)&&S&&(P=""),P&&Bt(P))await x(P);else{const C=await window.api.launchNextApp(t.localPath,t.type);await x(C)}return}if(t.type==="website"&&t.url){await x(t.url);return}if(t.type==="exe"&&t.localPath){Tn(`Launching ${o}...`);const P=await window.api.launchExe(t.localPath,t.tech);if(P&&P.mode==="embedded"&&P.url){const S=new URLSearchParams;P.token&&S.set("NL_TOKEN",P.token),t.tech&&S.set("TECH",t.tech);const C=String(P.url).split(":")[2];C&&S.set("NL_PORT",C);const L=`${P.url}?${S.toString()}`;await x(L)}else P?.success&&P.mode==="external"?Ke(`Opened "${o}" in a separate window.`,"info"):alert(`${o} launched externally. Embedded view is not available for this app.`);return}alert(`Cannot launch "${o}". Missing supported launch configuration.`)}catch(v){if(wn(t)){console.error("Failed to launch external Electron app from view:",v),Ke(`Failed to launch "${o}".`,"error");return}console.error("Failed to launch installed app from view:",v),alert(`Failed to launch "${o}": ${v.message||v}`)}finally{r()}}async function ri(e,n=null,t="New Tab",o=!1,r={}){const u=String(e||"").trim();if(!u)return;if(window.isSiteSnapStudioMode||window.parent?.isSiteSnapStudioMode){const F=be.gsk;window.enterSiteSnapStudioMode();const H=Yt(),ee=(H||[]).some(le=>{const V=String(le.url||"").trim();return V&&V!=="about:blank"&&V!=="newtab"});!H||H.length===0||o||ee?await Ae(u,F,t,null,r):await xt(u,F,t,null,r);return}let v=o;u.toLowerCase().startsWith("file://")&&(v=!0);const B=await Xn(u,n,t,r);if(!B||B.cancelled)return;const P=B.partition,S=B.profileId,C=B.lockedProfileId;S&&S!==Mi()&&ht(S,{bypassLock:!0});const L=Yt();if(!L||L.length===0){await Ae(u,P,t,C,r);return}const D=L.some(F=>{const H=String(F.url||"").trim();return H&&H!=="about:blank"&&H!=="newtab"});if(v||D){await Ae(u,P,t,C,r);return}let R=tt();if(!R){const F=L[L.length-1];F&&(await Ye(F.id),R=F)}if(!R){await Ae(u,P,t,C,r);return}await xt(u,P,t,C,r)}window.initViewPage=Yn;window.createTab=Ae;window.getTabs=Yt;window.getActiveWebview=Ge;window.launchInstalledAppFromView=Fo;window.openUrlFromDashboard=ri;function Wo(){const e=localStorage.getItem(et.currentProfileId);e&&se[e]?fe=e:fe="guest",ai();const n=document.getElementById("profileBtn"),t=document.getElementById("profileDropdown");n&&t&&(n.addEventListener("click",async o=>{o.stopPropagation(),t.classList.contains("hidden")?await lt(()=>t.classList.remove("hidden")):Ze(()=>t.classList.add("hidden"))}),t.innerHTML=Object.values(se).map(o=>`
    ...
    `).join("").concat(`
      ...
      `}).join(""),t=document.getElementById("addBookmarkBtn");t?t.insertAdjacentHTML("beforebegin",n):e.insertAdjacentHTML("beforeend",n)}function Yo(){const e=document.getElementById("addBookmarkBtn"),n=document.getElementById("bookmarkCurrentPageHeaderBtn"),t=document.getElementById("bookmarkModal"),o=document.getElementById("bookmarkModalCloseBtn"),r=document.getElementById("bookmarkForm"),u=document.getElementById("bookmarkTitleInput"),v=document.getElementById("bookmarkUrlInput"),x=t?.querySelector(".password-modal-header h3"),B=document.getElementById("bookmarkSaveBtn");if(!e||!t||!o||!r||!u||!v)return;const P=()=>{Ze(()=>{t.classList.add("hidden"),Pt=null})},S=async({editId:D=null,title:R="",url:F="",profileId:H=fe,heading:ee="Add Bookmark",saveLabel:le="Save Bookmark"}={})=>{Pt=D,dt=se[H]?H:fe,on("bookmarkProfileSelect",dt,V=>{dt=V}),x&&(x.textContent=ee),B&&(B.textContent=le),r.reset(),u.value=String(R||""),v.value=String(F||""),await lt(()=>t.classList.remove("hidden")),u.value?(u.focus(),u.select()):u.focus()},C=async()=>{await S()},L=async()=>{const D=tt(),R=wt(D?.url||"");if(!D||D.isHome||!R||R==="about:blank"){Ke("Open a website tab first to save it as a bookmark.","error");return}const F=jo(D),H=Vo(R,F);await S({editId:H?.id||null,title:D.title||H?.title||"New Bookmark",url:R,profileId:F,heading:H?"Update Bookmark":"Save Current Site",saveLabel:H?"Update Bookmark":"Save Bookmark"})};e.addEventListener("click",C),e.addEventListener("keydown",async D=>{(D.key==="Enter"||D.key===" ")&&(D.preventDefault(),await C())}),n?.addEventListener("click",L),n?.addEventListener("keydown",async D=>{(D.key==="Enter"||D.key===" ")&&(D.preventDefault(),await L())}),o.addEventListener("click",P),t.addEventListener("click",D=>{D.target===t&&P()}),r.addEventListener("submit",D=>{D.preventDefault();const R=String(u.value||"").trim(),F=wt(v.value);if(!R||!F)return;const H=qo({id:Pt,title:R,url:F,profileId:dt||fe});H&&(ci(),sn(),P(),r.reset(),Ke(H==="updated"?"Bookmark updated successfully.":"Bookmark saved successfully.","success"))})}async function Ko(e){const n=document.getElementById("bookmarkModal"),t=document.getElementById("bookmarkForm"),o=document.getElementById("bookmarkTitleInput"),r=document.getElementById("bookmarkUrlInput"),u=n?.querySelector(".password-modal-header h3"),v=document.getElementById("bookmarkSaveBtn");if(!n||!t||!o||!r)return;const x=Oe.find(B=>B.id===e);x&&(Pt=x.id,dt=x.profileId||fe,u&&(u.textContent="Edit Bookmark"),v&&(v.textContent="Update Bookmark"),on("bookmarkProfileSelect",dt,B=>{dt=B}),o.value=x.title||"",r.value=x.url||"",await lt(()=>n.classList.remove("hidden")),o.focus())}function Go(){const e=document.getElementById("tabsList"),n=document.getElementById("tabsScrollLeft"),t=document.getElementById("tabsScrollRight");if(!e||!n||!t)return;const o=220;n.addEventListener("click",()=>{e.scrollBy({left:-o,behavior:"smooth"})}),t.addEventListener("click",()=>{e.scrollBy({left:o,behavior:"smooth"})}),e.addEventListener("scroll",Dt),Dt()}function di(){const e=document.getElementById("historyList"),n=document.getElementById("historyProfileSelect");if(!e||!n)return;const t=zt||fe,o=Te[t]||[];if(o.length===0){e.innerHTML="<div class='password-meta'>No history for this profile yet.</div>";return}const r=new Date,u=new Date(r.getFullYear(),r.getMonth(),r.getDate()).getTime(),v=u-864e5,x={today:[],yesterday:[],older:[]};o.forEach((L,D)=>{const R={...L,originalIndex:D},F=L.visitedAt||0;F>=u?x.today.push(R):F>=v?x.yesterday.push(R):x.older.push(R)});const B=L=>L.toLocaleDateString(void 0,{month:"short",day:"numeric"}),P=`Today - ${B(r)}`,S=`Yesterday - ${B(new Date(v))}`,C=(L,D,R=!1)=>{if(D.length===0)return"";const F=D.map(H=>`
      <div class="history-item" data-index="${H.originalIndex}">
        <div class="history-main">
          <div class="history-title">${Ce(H.title||"Untitled")}</div>
          <div class="history-url">${Ce(H.url||"")}</div>
          <div class="password-meta">${Ce(oi(H.visitedAt))}</div>
        </div>
  ...
  `,e.querySelectorAll(".history-item").forEach(L=>{L.addEventListener("click",D=>{const R=D.target.closest("button");if(!R)return;const F=Number(L.dataset.index);if(Number.isNaN(F))return;const H=Te[t]||[],ee=H[F];if(ee){if(R.dataset.action==="delete"){H.splice(F,1),Te[t]=H,rn(),di();return}if(R.dataset.action==="open"){const le=se[t]?.partition||be.guest;Ae(ee.url,le,ee.title||"History",t)}}})})}async function at({title:e="Clear Page Cache",message:n="",confirmLabel:t="OK",cancelLabel:o="Cancel",hideCancel:r=!1}={}){const u=document.getElementById("cacheActionModal"),v=document.getElementById("cacheActionTitle"),x=document.getElementById("cacheActionMessage"),B=document.getElementById("cacheActionCloseBtn"),P=document.getElementById("cacheActionCancelBtn"),S=document.getElementById("cacheActionConfirmBtn");return!u||!v||!x||!B||!P||!S?Promise.resolve(window.confirm(n||e)):(v.textContent=e,x.textContent=n,S.textContent=t,P.textContent=o,P.style.display=r?"none":"inline-flex",await lt(()=>u.classList.remove("hidden")),new Promise(C=>{const L=()=>{B.removeEventListener("click",D),P.removeEventListener("click",D),S.removeEventListener("click",R),u.removeEventListener("click",F),Ze(()=>u.classList.add("hidden"))},D=()=>{L(),C(!1)},R=()=>{L(),C(!0)},F=H=>{H.target===u&&D()};B.addEventListener("click",D),P.addEventListener("click",D),S.addEventListener("click",R),u.addEventListener("click",F)}))}function Dt(){const e=document.getElementById("tabsList"),n=document.getElementById("tabsScrollLeft"),t=document.getElementById("tabsScrollRight");if(!e||!n||!t)return;if(!(e.scrollWidth>e.clientWidth+1)){n.classList.add("hidden"),t.classList.add("hidden"),e.scrollLeft=0;return}const r=e.scrollLeft<=1,u=e.scrollLeft+e.clientWidth>=e.scrollWidth-1;n.classList.toggle("hidden",r),t.classList.toggle("hidden",u)}function an(e){if(!e)return null;if(typeof e.getWebContentsId=="function")try{const n=e.getWebContentsId();if(n)return n}catch{}return e._webContent?e._webContent.key||e._webContent.id||null:e.key||e.id||e.getAttribute("key")||e.getAttribute("id")||null}async function Jo(e,n){const t=re.findIndex(r=>r.id===n),o=re[t];if(e==="duplicate-tab"&&o){io(n);return}if(e==="inspect-local-file"&&o){const r=document.getElementById(`webview-${o.id}`),u=an(r);if(!u||!window.api?.toggleWebviewDevTools){Ke(At?"Inspect is not available for this tab.":"Inspect is not available for this local file tab.","error");return}await window.api.toggleWebviewDevTools(u)||Ke(At?"Could not open developer tools.":"Could not open developer tools for this local file.","error");return}if(e==="clear-cache"&&o){const r=document.getElementById(`webview-${o.id}`),u=r&&typeof r.getURL=="function"&&r.getURL()||o.url||"",v=r&&r.getAttribute("partition")||o.partition||be.guest;if(!u||u==="about:blank"||!await at({title:"Clear Page Cache",message:`Clear cache and site data for ${u}?`,confirmLabel:"Clear Cache",cancelLabel:"Cancel"}))return;try{if(!window.api||typeof window.api.clearWebviewPageCache!="function"){await at({title:"Action Unavailable",message:"Cache clear API is not available in this app session. Please restart OneView and try again.",confirmLabel:"OK",hideCancel:!0});return}const B=await window.api.clearWebviewPageCache(v,u);B?.success?r&&(typeof r.reloadIgnoringCache=="function"?r.reloadIgnoringCache():r.reload()):await at({title:"Could Not Clear Cache",message:B?.message||"Unknown error",confirmLabel:"OK",hideCancel:!0})}catch(B){const P=String(B?.message||B||""),S=/No handler registered for 'clear-webview-page-cache'/.test(P)?" Restart OneView completely so the latest main-process IPC handlers load.":"";await at({title:"Could Not Clear Cache",message:`${P}${S}`,confirmLabel:"OK",hideCancel:!0})}return}if(e==="clear-user-data"&&o){const r=document.getElementById(`webview-${o.id}`),u=r&&typeof r.getURL=="function"&&r.getURL()||o.url||"",v=r&&r.getAttribute("partition")||o.partition||be.guest;if(!u||u==="about:blank"||!await at({title:"Clear User Data",message:`Clear local storage and site data for ${u}?`,confirmLabel:"Clear Data",cancelLabel:"Cancel"}))return;try{if(!window.api||typeof window.api.clearWebviewUserData!="function"){await at({title:"Action Unavailable",message:"User-data clear API is not available in this app session. Please restart OneView and try again.",confirmLabel:"OK",hideCancel:!0});return}const B=await window.api.clearWebviewUserData(v,u);B?.success?r&&(typeof r.reloadIgnoringCache=="function"?r.reloadIgnoringCache():r.reload()):await at({title:"Could Not Clear User Data",message:B?.message||"Unknown error",confirmLabel:"OK",hideCancel:!0})}catch(B){const P=String(B?.message||B||""),S=/No handler registered for 'clear-webview-user-data'/.test(P)?" Restart OneView completely so the latest main-process IPC handlers load.":"";await at({title:"Could Not Clear User Data",message:`${P}${S}`,confirmLabel:"OK",hideCancel:!0})}return}if(e==="clear-all"){mt(re.map(r=>r.id));return}if(e==="clear-right"&&t>=0){mt(re.slice(t+1).map(r=>r.id));return}e==="clear-left"&&t>=0&&mt(re.slice(0,t).map(r=>r.id))}function Xo(){const e=document.querySelector(".tabs-header");e&&e.addEventListener("contextmenu",async n=>{if(n.target.closest(".profile-section"))return;n.preventDefault();const r=n.target.closest(".tab")?.id?.replace("tab-ui-","")||Ve||re[0]?.id||null;if(!r)return;Rn=r;const u=re.findIndex(L=>L.id===r),v=re[u],x=document.getElementById(`webview-${r}`),B=ro(v,x),P=!!(v&&!v.isHome&&(x&&x.getURL()!=="about:blank"||v.url)),S=u>0?u:0,C=u>=0&&u<re.length-1?re.length-u-1:0;window.api&&typeof window.api.showNativeTabContextMenu=="function"&&await window.api.showNativeTabContextMenu({anchorId:r,x:Math.round(n.x),y:Math.round(n.y),disabled:{clearLeft:S===0,clearRight:C===0,clearCache:!P,clearUserData:!P,inspectLocalFile:!B}})})}function Vt(e){if(!e)return"";const n=String(e.getData("text/uri-list")||"").split(/\r?\n/).map(u=>u.trim()).find(u=>u&&!u.startsWith("#"));if(n&&/^https?:\/\//i.test(n))return n;const o=String(e.getData("text/html")||"").match(/\bhref\s*=\s*['"]([^'"]+)['"]/i);if(o&&/^https?:\/\//i.test(String(o[1]||"").trim()))return String(o[1]||"").trim();const r=String(e.getData("text/plain")||"").trim();return/^https?:\/\//i.test(r)?r:r&&!/\s/.test(r)&&/\./.test(r)?wt(r):""}function Qo(){const e=document.querySelector(".tabs-header");if(!e||e.dataset.dropBound==="1")return;e.dataset.dropBound="1";const n=t=>{e.classList.toggle("is-drop-target",!!t)};e.addEventListener("dragenter",t=>{Vt(t.dataTransfer)&&(t.preventDefault(),n(!0))}),e.addEventListener("dragover",t=>{Vt(t.dataTransfer)&&(t.preventDefault(),t.dataTransfer&&(t.dataTransfer.dropEffect="copy"),n(!0))}),e.addEventListener("dragleave",t=>{e.contains(t.relatedTarget)||n(!1)}),e.addEventListener("drop",t=>{const o=Vt(t.dataTransfer);if(n(!1),!o)return;t.preventDefault();const u=t.target.closest(".tab")?.id?.replace("tab-ui-","")||Ve,v=re.findIndex(x=>x.id===u);Ae(o,null,"New Tab",null,{insertIndex:v>=0?v+1:re.length})})}function Zo(e){const n=document.getElementById("profileBtn"),t=document.getElementById("profileDropdown");!n||!t||!n.contains(e.target)&&!t.contains(e.target)&&!t.classList.contains("hidden")&&Ze(()=>t.classList.add("hidden"))}async function er(e){const n=String(e?.action||"");if(!n||n==="__menu_closed__")return;const t=String(e?.anchorId||Rn||Ve||re[0]?.id||"");t&&await Jo(n,t)}const Un={desktop:{width:1920,height:1080,userAgent:"desktop"},mobile:{width:414,height:896,userAgent:"mobile"},tablet:{width:768,height:1024,userAgent:"tablet"}};async function ui(e,n="mobile"){if(!e)throw new Error("No active webview");const t=Un[n]||Un.mobile;if(console.log(`[Viewport] Setting ${n} viewport: ${t.width}x${t.height}`),window.api?.setWebviewBounds){const o=an(e);o&&(console.log(`[Viewport] Triggering native resize to ${t.width}x${t.height} for id: ${o}`),await window.api.setWebviewBounds(o,{width:t.width,height:t.height}))}return window.__oneview_original_webview_dims||(window.__oneview_original_webview_dims={width:e.style.width,height:e.style.height,minWidth:e.style.minWidth,minHeight:e.style.minHeight,maxWidth:e.style.maxWidth,maxHeight:e.style.maxHeight,flex:e.style.flex}),e.style.width=t.width+"px",e.style.height=t.height+"px",e.style.minWidth=t.width+"px",e.style.minHeight=t.height+"px",e.style.maxWidth=t.width+"px",e.style.maxHeight=t.height+"px",e.style.flex="none",console.log(`[Viewport] Resized webview element to ${t.width}x${t.height}`),await e.executeJavaScript(`
    ...
    `,!0),await new Promise(n=>setTimeout(n,500))}async function nr(e={}){const n=Ge();if(!n||typeof n.executeJavaScript!="function")throw new Error("Active tab is unavailable");if(typeof n.capturePage!="function")throw new Error("Active tab does not support capture");const t=String(e?.viewport||"desktop").trim().toLowerCase();if(console.log("[Capture] Active webview found",{id:n.id,url:n.getURL?.(),title:n.getTitle?.(),viewport:t,loading:n._webContent?.state?.loading}),t==="mobile"||t==="tablet"){const V=t==="tablet"?"tablet":"mobile";await ui(n,V),console.log("[Capture] Viewport changed to "+V+", waiting for page reflow..."),await new Promise(ae=>setTimeout(ae,300))}const o=5e3,r=Date.now();for(;n._webContent?.state?.loading&&Date.now()-r<o;)console.log("[Capture] Waiting for page to load..."),await new Promise(V=>setTimeout(V,200));console.log("[Capture] Page load status:",n._webContent?.state?.loading?"still loading":"loaded");const u=String(e?.mode||"visible").trim().toLowerCase();if(u!=="full"&&u!=="fullpage"){const V=await n.capturePage(),ae=V?.isEmpty?.()?"":V.toDataURL();return console.log("[CapturePage] Visible captured. DataUrl length:",ae?.length||0),{mode:"visible",dataUrl:ae,width:V?.getSize?.()?.width||0,height:V?.getSize?.()?.height||0}}const v=await n.executeJavaScript(`
      (() => {
  ...
  `,!0).catch(()=>{}),x>B+100){console.log("[FullCapture] Verifying scroll functionality...");const ae=await n.executeJavaScript(`
    ...
    `,!0).catch(()=>{})}const P=Math.floor(B*.8),S=Math.ceil(x/P);console.log("[FullCapture] Progressive scroll: "+S+" steps, "+P+"px per step");let C=0;for(let V=0;V<S;V++){C=Math.min(C+P,x),console.log(`[FullCapture] Scrolling host-driven to ${C}px (${V+1}/${S})`);const ae=`
      (() => {
        const targetY = ${C};
        try {
    ...
    `;await n.executeJavaScript(ae,!0).catch(()=>{}),await new Promise(Ie=>setTimeout(Ie,600))}await n.executeJavaScript(`window.scrollTo({ top: ${x}, left: 0, behavior: 'auto' });`,!0).catch(()=>{}),await new Promise(V=>setTimeout(V,800)),await n.executeJavaScript(`
    (() => {
  ...
  `,!0).catch(()=>{}),console.log("[FullCapture] Progressive scroll and freeze complete, back at top.");const F=Number(e?.wait||0)*1e3,H=200+F;console.log(`[FullCapture] PHASE 2: Scrolling back to top complete. Waiting ${H}ms (Base 0.2s + User ${F}ms) for page to settle live...`),await new Promise(V=>setTimeout(V,H));let ee="",le=null;try{console.log("[FullCapture] Calling native one-shot capture..."),le=await n.capturePage({mode:"full",scrollHeight:Math.round(x)}),ee=le?.isEmpty?.()?"":le.toDataURL()}finally{console.log("[FullCapture] Restoring original body and globals..."),await n.executeJavaScript(`
        (() => {
